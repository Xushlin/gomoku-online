...
Before changing what a control looks like, read **`fix-primary-label-contrast`**: the theme contrast requirement was green for months while 25 of 31 primary buttons painted their label with `--color-bg` — a **background** token used as a foreground, a pairing the check's domain never included (measured 1.58:1 in qq-game dark). The check now derives pairings from the class attributes themselves, and forbids two foreground utilities on one element, because a same-specificity override is invisible: one `[class.text-bg]` had **never once applied**. **A check can measure exactly the right thing while the application does something else.**

For anything that draws a **glyph inside a shape**, read **`add-game-emblems`** first: the queryable box is not the visible ink (a CJK SVG `<text>` line box is exactly font-size wide but ~1.45× tall), a stroked container's usable interior is `radius − strokeWidth / 2`, and the only measurement that answers "does it fit" is drawing to a canvas and sampling ink pixels. Three measurements, and the first two disagreed with the screenshot.
