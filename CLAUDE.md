...
- [x] **`enable-xiangqi-human-play`** — 象棋 is a full human-vs-human rated game. **Its own doc comment predicted this change and named the trigger** — 「大厅泛化之后翻它,而计不计分是那时一个独立的、需要理由的决定」 — the third self-triggering deferral in this repo and the cleanest, because it also said what would still need deciding.
  ...
  **The verification found a pre-existing authorization defect and did not fix it.** `in-room-chat` says the Spectator channel is visible to spectators only. Writing is enforced; *reading* leaks on **all three** paths. `GET /api/rooms/{id}` returns every message regardless of caller; the `RoomState` broadcast pushes one DTO to the whole room group; and `ChatMessage` events — whose per-channel routing *is* correct — are reachable anyway, because **`JoinSpectatorGroup` validates nothing**: a player joins the spectator subgroup just by asking. So spectator-channel confidentiality rests entirely on client honesty, and the only thing the server truly enforces is that players cannot *post* there.

  **I first called that third path correct, and I had only read it.** The per-channel routing is right; *joining* the group is unchecked, and reading half a mechanism cannot show you the other half. The wrong call had a cost: it graded the defect a level too low — "the snapshot path lagged" rather than "the rule is unenforced." That is the third time for this same lesson (`add-hub-error-codes`' wrapper format, `remove-manifest-board`' first frame, this one): **a conclusion from reading and a conclusion from measuring differ only when they differ — and they often do.** `ChatPanel` hiding the spectator tab is why the screen looks right while the data sits in the player's client.

  Present since `in-room-chat`; 象棋 did not make it reachable, only visible, because reproducing it needs human-vs-human *and* spectators at once. Fixed separately as a pure spec-compliance bug — see the roadmap.

Not yet done — platform roadmap:

1. **`fix-spectator-chat-leak`** — the defect above, on all three paths. `JoinSpectatorGroup` must check membership (which also closes a spec/implementation gap: `room-and-gameplay` says `JoinRoom` decides the subgroup from the aggregate, while the implementation lets the client self-declare); `ToState` must trim by caller; and the `RoomState` broadcast must split, since there is no players-only SignalR group today — that last one touches `web-game-board`'s reconnect protocol. Pure bug fix against an existing spec, so no proposal needed.
