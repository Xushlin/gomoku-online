...
- `ng build` still reports `bundle initial exceeded maximum budget`, and `add-web-tetris` did **not** make it worse: the whole game is in lazy chunks (`play` and `score-leaderboard-page`), so the initial bundle is unchanged. It was 350 bytes over (500.35 kB against a 500 kB budget) after `generalize-lobby`. It is a warning, not an error, so CI is green. Closing it needs one small thing rather than an architectural change; be careful not to "fix" it by raising the budget, which converts a live signal into silence.
