...
1. `add-web-per-game-rating` — the front-end half of per-game rating. The backend deliberately shipped with the published client untouched: `gameKey` defaults to `gomoku` in every controller, so `/api/leaderboard`, `/api/users/{id}`, `/api/users/me` and the auth responses all still serve exactly the gomoku numbers. The cost is that the profile page can only show one game and the leaderboard has no game switcher. That is a **known gap, not an oversight** — but it is the gap that grows every time another rated game ships.
