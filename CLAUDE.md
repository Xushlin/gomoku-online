...
- **The theme is two orthogonal axes, the list is derived from that artefact, and the board colour is not a third axis.** `SettingsRepository` holds a name and a brightness separately (flattening them is how 「切成浅色」starts silently resetting the theme — measured: a control that made `setDark` rewrite the name reds exactly one of the two direction tests); the settings screen names no theme, it walks `themeTokens.keys`; and `AppTheme.boardBackground` reads `color-well` out of the **same** token bag, so choosing a theme *is* choosing the board colour — pinned by a test that four themes give more than one board colour, which is the day a real skin axis would be needed. **The defect this shipped to fix was not a missing capability:** all four themes painted fine, `GewuApp` just passed literal `ink`/`dark` to `MaterialApp.router`. Every storage assertion is green on that version, so the criterion has to be the **painted `ThemeData` under the real shell** — a unit test that mounts the settings page alone cannot see it.
