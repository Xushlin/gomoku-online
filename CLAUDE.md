...
| **回放表达不出三个座位,而它已经在丢数据。** `GameReplayDto` 只有 `Black` / `White` 两个字段,`GetGameReplayQueryHandler` 无条件读 `room.BlackPlayerId` / `WhitePlayerId` —— 所以**一局已结束的斗地主 / 挖坑,它的回放静默丢掉 2 号座位**,并把另两人叫成黑白。没有任何测试覆盖三座位回放(`GetGameReplayQueryHandlerTests` 里 0 处提到三座位)。`per-game-seat-labels` 只修了它**表达得出来**的那一半(两位玩家的席位名),因为剩下那一半是契约(DTO + handler + 前端 + 测试),混进一个显示层变更里会同时改两层。**一个单独的变更正在做这件事** —— 若它先落地,这一行连同 `web-replay` 那条要求的手工合并一起消掉。 | 那个变更合并;或者它没做完而有人发现三座位回放少一个人。 |
