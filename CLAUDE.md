...
1. **`add-tetris` — Domain layer done, Application/Api/persistence not started.** See `openspec/changes/add-tetris/`. What remains: the `ScoreRun` entity + migration, two commands (start / submit), one periodic-leaderboard query, and the real-HTTP verification. Stopping there was deliberate — those form one complete layer, and a half-done one (an entity with no migration) is worse than none.
...
- Chat content is capped at 500 characters server-side, and a 400-character unbroken string only stays inside a 375 px panel because of `overflow-wrap: break-word`. **Nothing in the test suite guards that** — jsdom has no layout, so the only evidence is a browser run. If the chat panel's styles are ever rewritten, the regression is a horizontally-scrolling room page that no unit test will catch.
...
- `logs/gomoku-.log` Serilog filename — normative in `observability` and **matching the implementation**, so this one is purely a gomoku→gewu naming wart, not a trap. It rides along with the other renames.
