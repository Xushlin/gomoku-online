...
**Ten games ship, and the catalogue now has no `planned` entry left.** They fall into three categories that deliberately do **not** share one aggregate:
...
| The per-seat broadcast fan-out has no end-to-end test — projection is unit-tested and the group function is exhaustive by construction, but "three real SignalR connections each receive only their own" is unasserted, so a typo in `ViewGroupName` turns nothing red. **The old trigger has fired**: `Gewu.Api.Tests` now exists (`fix-urge-user-routing` needed it), and it does not cover this — it holds one pure claim-reading test, not a host. **What the absence actually cost has now been measured once:** `Clients.User(...)` addressed every 催促 to nobody for the platform's whole life, silently, and the thing that found it was a *client* probe. | A `WebApplicationFactory` harness exists — i.e. the day any test here opens a real hub connection. Until then, `frontend-mobile/test/room_social_probe_test.dart` is the only thing that watches a push actually arrive, and it is **opt-in** (`GEWU_PROBE_SERVER`), so CI is green whether or not it ran. |
...
| **手机端退出确认弹窗的标题和确认按钮是同一句「退出登录」,而这是约束不是疏忽。** 这个客户端不许自造翻译键(`shared_sync_test` 就是干这个的),而共享文案里没有通用的「确定」、也没有退出确认的正文 —— web 端**根本不问**。代价是屏幕上同时有三个「退出登录」,查找器只能收到 `find.widgetWithText(TextButton, …)` 上。 | web 端也给退出加确认 —— 那时正文和确认按钮的文案自然会存在,两端一起改。 |
...
- **In a Flutter `testWidgets` body, a real HTTP call never returns, and it does not look like a hang.** The body runs on a fake clock; Dio arms a **real** `Timer` for its send timeout, so `await login(...)` there simply never completes — and because `pumpAndSettle` advances the *fake* clock, the 30-second test timeout never fires either. What you see is one test burning CPU until `pumpAndSettle`'s **ten-minute** ceiling, which reads like an animation that will not settle rather than a stalled future. Wrap it in `tester.runAsync`. **And do not shortcut it by assigning the fixture's fields**: `AuthRepository.signedIn` is a private notifier only `_adopt` flips, and it is what the router reads — a stub that sets `currentUser` alone makes 「取消没有登出」 green while never having been signed in.
- **`GET /api/rooms` and `GET /api/rooms/{id}` are different shapes, and the lobby only ever sees the smaller one.** The summary carries **no `seatCount`** and its `seats` lists **only the taken seats**, so `takenSeats < totalSeats` degenerates to `1 < 1` and every room in the lobby reads as full — including the empty ones. Measured after shipping it: every unit test stayed green because the fixture was a **full room JSON**, complete with `seatCount`, which that screen never receives. An unrelated integration test caught it. **A fixture the screen cannot receive proves nothing about the screen** — for anything the lobby list renders, build the fixture from `RoomSummaryDto`'s actual fields. The seat total for a listed room comes from the game descriptor (the lobby is per-game), never from the summary.
- **`GET /api/rooms` has no `Take` and no `OrderBy`, so a freshly created room is at the *bottom* of the lobby — and a `ListView` only builds what is visible.** An integration test that tapped its own room by name went red once the scratch database had accumulated 20 rooms, and the message was "found 0 widgets", which reads as *the room was not listed*. It was listed. Anything that taps a row in a list this client renders must `scrollUntilVisible` first — the claim under test is never "it happened to fit on one screen".
- **A row whose height depends on whose turn it is will move the board every ply, and it reads as a flicker rather than as a layout bug.** The action bar's 「why you cannot urge」 line is present on your turn and absent on your opponent's, so the centred board inside `Expanded` resized by **20 px** on every single move — in both games, which is the clue that it is not in either renderer. The criterion is a **`Rect` compared across a turn change**; `GameView` had no unit test at all before that, only integration tests that drive the whole app and never compare geometry. Reserve the space (`Visibility` with `maintainSize`) rather than deleting the explanation, and overlay transient messages instead of inserting them as rows — a `SnackBar` is the wrong overlay here because it dismisses itself and an integration test asserts the refusal is on screen.
...
- **`openspec validate --strict` validates spec *shape*, never spec *truth* — and not even all of the shape it advertises.** It was 37/37, 38/38 and 41/41 green across every drift this repo has found, including a Scenario that had never been implemented (four occurrences) and a live spec contradicting the code for 36 commits. **Measured on v1.3.0:** breaking *all 8* `#### Scenario:` headers in a delta passes; breaking all 15 in another and flipping `MODIFIED` to `ADDED` passes too — while its own failure text says "Each requirement MUST include at least one `#### Scenario:` block". Only a delta file with **no delta section header at all** goes red. So it checks that the file has an `## ADDED/MODIFIED Requirements` line and essentially nothing inside it. **A positive control that mutates one Scenario is not a control** — it passes because the requirement still has others, which is exactly how this was nearly missed. **The signal is a merged PR whose change directory is still in `openspec/changes/`** — check that list.
- **Archive in merge order, and that is necessary but not sufficient.** MODIFIED replaces a requirement wholesale, so when two unarchived changes touch one requirement, one must be hand-merged. Generate MODIFIED bodies by *extracting from the live spec and patching*, never by retyping — retyping is how an unrelated sentence gets silently reverted. **Wholesale means the trailing `---` separator too:** a MODIFIED body that does not end with one silently deletes it from the live spec, and nothing red ever appears. Renaming a requirement needs a `RENAMED` block or archive aborts. **The cheap way to know the archive did what you meant:** snapshot the live spec files first, then compare requirement-header counts, `---` counts and every removed line against what you predicted — `history-every-seat` predicted 11 removals and the archive made 14, and the three extra were two separators and a blank.
- **A PR can be `MERGED` on GitHub and its code still never reach `main`.** `history-every-seat` targeted `fix/replay-every-seat`; that base had been merged to `main` **an hour earlier**, so the stacked PR merged into a branch that no longer flowed anywhere. GitHub reports a green MERGED either way — **the merge succeeded, it just went nowhere.** It cost a whole change silently not shipping, and the missing CI was explained away as normal for a stacked PR, which is true and which hid it. **Never read the PR's state; read `main`.** `git merge-base --is-ancestor <sha> origin/main`, or grep `main` for a symbol the change introduced. Note that a rebase-merge rewrites the SHAs, so `git log origin/main..origin/<branch>` shows content that *is* in `main` as "not in main" — compare **trees** (`git diff --stat <main-sha> <branch-base>` empty = identical), not SHAs. When stacking, merge the bottom PR last, or retarget the top one to `main` the moment the bottom lands.
...
### Mobile (`frontend-mobile/`) — Flutter, two games shipped

Material 3 + `signalr_netcore`. **Shell + auth + 五子棋 + 中国象棋**, with a catalogue in front of them; the first slice was verified on a real Android emulator (register, room, move, server records it). The other eight games are deliberately absent — one at a time, like the web client, and the catalogue lists them disabled rather than hiding them.
...
- **Sound is the one thing here that cannot be pulled from `frontend-web`, and that turned into the sharpest criterion on this client.** Web's three packs are synthesised with WebAudio at play time, so the repo holds **no audio files** — what gets ported is the design (frequencies, durations, gains in `packs/minimal.ts`), and the event set is *derived* from `sound.tokens.ts` by a walk asserting set **equality**. Synthesising rather than bundling is what makes it testable: a `.wav` can only be asserted to exist, but generated PCM has a measurable **dominant frequency** — `test/sound_test.dart` measures 660 Hz out of the stone click with Goertzel, and C5/G4 out of win/lose **separately** (asserting only that they differ would pass an implementation that swapped them). **No assertion here depends on anybody hearing anything**, which is also the limit: whether sound actually comes out is a device check, not a test. Muting means *not calling the player* — playing at zero volume looks identical on a quiet phone and still takes audio focus, pausing whatever the person was listening to.
- **Five axes now — theme, brightness, sound, skin, language — and each landed the same way: the capability already existed with no way to reach it.** Both locales have shipped in the bundle since the first commit; `AppDependencies` hard-coded one and loaded it once. **Switching needs the whole tree to get a new `Translations` instance**: it is loaded asynchronously, `Provider.value` handed a new instance rebuilds nothing by itself, and `GewuApp` must stay a `StatelessWidget` — so the reload lives in a repository and the provider sits *inside* the shell's listenable builders. That works only because no view captures `Translations` in `initState` (checked before relying on it). The device's language is a **fallback, never a source**: once somebody chooses, a system-language change must not overrule it. The skin axis overturned a written `MUST NOT`. `add-mobile-settings` recorded that the board's colour follows the theme and that this client must not grow a skin axis, *with its own dismantling condition*; a request for web-parity skins met it, so the rule was **rewritten via `MODIFIED`** rather than contradicted in silence. Skins are a synced artefact (`skin_tokens.g.dart` from `board-skins.css`) whose **gradient text is kept verbatim** — dropping it would make drift undetectable, and it is what `board_gradient_test.dart` parses and paints. Two CSS conventions are pinned because getting either wrong is silent: **the first `background-image` layer is on top** (Flutter paints in the reverse order, and getting it backwards hides the wood grain under its own base colour), and **`90deg` is to the right, not up**. The three skins are genuinely different kinds of thing: `wood` and `midnight` are literals plus gradients, while **`classic` is written entirely in `var()`/`color-mix()` — it *is* "follow the theme"**, which is what this client did before. Parity is measured by sampling painted pixels against the CSS text, **not** against a browser screenshot.
...
lib/
  ...
  ui/game/board_registry.dart  the ONLY source of "which games can this client draw"
  ui/<feature>/
...
**The game catalogue comes from `GET /api/games`, and the client keeps no game table** — board size, seat count and `isRated` all arrive as `GameDescriptorDto`, because a wrong `isRated` copy shows up as *a leaderboard that is always empty*, which looks exactly like a game nobody has played. The board's size is read from the **room's own `gameKey`**, never from the route path: only one of the four ways into a room leaves the client already knowing the game. `ui/game/board_registry.dart` is the only source of which games have a painter here, and the catalogue disables the rest rather than hiding them. **One measured filter is allowed and it is not a second table:** `GET /api/games` serves 7 games and one of them (`xiangqi-endgame`) has no title in the shared i18n artefact — it is not a browsable game on the web either — so entries without copy are dropped, derived from the bundle `shared_sync_test` already pins. Routes are three deep and nested (`/` → `/games/:key` → `/games/:key/rooms/:id`); flattening **either** level compiles, analyses clean and silently returns `canPop()` to false, measured twice.

**The mobile client's entire INBOUND hub half was dead from day one, and every test was green.** It subscribed to `RoomStateChanged`; the server sends `RoomState`. SignalR ignores a subscription to a name nobody invokes — silently, with no error on either end — so a joined game kept saying 等待中 and a played move never appeared on the board. Nothing caught it because **every test asserted the *server's* state over REST**, and the board people saw was the one-shot REST snapshot `RoomRepository.open` takes. Outbound worked, which is what made it look fine. `test/hub_contract_test.dart` now derives the legal names from `backend/src/Gewu.Api/Hubs/SignalRRoomNotifier.cs` itself. **Note what the original probe proved:** `test/hub_probe_test.dart` established "the transport works" with `JoinRoom` + `MakeMove` — **both outbound**. A connectivity proof that only sends is half a proof.

**A finished game is read from the room snapshot, and from nothing else.** The server puts `Result` / `WinnerUserId` / `EndReason` in every snapshot and the client used to parse none of them, so a finished game on a real phone just stopped — board still there, every tap refused, nothing said. Measured from the handlers: `SaveChangesAsync` → `RoomStateChangedAsync` → `GameEndedAsync`, so the final `RoomState` already carries the result and the `GameEnded` push is redundant — it and its consumer-less stream were **deleted**. Win or lose compares **user ids**; `Decided` with a null winner must not read as a win. And `GameResult` is `Ongoing=0, Decided=1, Draw=3` — **no 2** — so it is parsed by name, and a *missing* `result` means "not finished" while an *unrecognised* one means "do not guess".

**Both lobby entries are derived from the descriptor, and the AI's first move arrives as a push.** `supportsHumanVsHuman` gates create-room and `supportsAi` gates the machine — never a list, because this lobby already shipped the failure that rule prevents: the create button was unconditional while `POST /api/rooms {"gameKey":"tictactoe"}` answers **400 'tictactoe' has no human-vs-human mode on this platform.** And **一字棋 needs no board of its own** — the server registers it as `NInARowRules("tictactoe", 3, 3, 3)`, so two registry keys share one renderer and the star points already derive to none at 3×3. **Measured, and it is the sharpest criterion here:** creating an AI room answers with `moves: 0` even when the bot holds seat 0 — it moves seconds later, so its first move reaches the screen only as a hub push. Picking White before `fix-mobile-hub-inbound` would have shown a board that stayed empty forever.

**Leaving a room has to tell the server, and which route is the server's rule.** `POST /api/rooms/{id}/leave` normally; **`DELETE /api/rooms/{id}`** for the host of a *waiting* room, who is otherwise refused with 409. The client had never called either — seven `leave`/`dissolve` hits in `lib/` were all prose — so backing out popped a route and left you seated forever. **Two criteria written for that fix were wrong and the unit tests were green for both:** the dissolve route (a fake adapter accepts any URL, so a test asserting the client sent what the client was written to send knows nothing), and "the seat is empty afterwards" (**measured: leaving never vacates a seat**, so the only server-observable criterion is the branch — the room is gone, or it is not). `test/room_route_contract_test.dart` now derives the legal verb+template set from `RoomsController`'s own attributes. Also measured: a two-seat room turns `Playing` the moment the second player joins, so a "waiting room" is one with an empty seat.

**Two more things only a real device showed**, both invisible to a missing-key walk because neither is a missing key: the room list printed `RoomStatus.wire` (the server's English `Playing`) into a Chinese UI, and two screens shipped a bare `Text('↻')`. `test/i18n_keys_test.dart` now also fails on any hard-coded `Text('…')` under `ui/` and on any use of `.wire` there. The device trip is where the system back button was finally *pressed*, too: room → lobby → catalogue → launcher, three presses, three expected outcomes.

**中国象棋's board is replayed by the client, and replaying is not judging.** The server sends no position (`GameSnapshotDto` carries only `Moves`), so `ui/game/xiangqi/position.dart` applies `from → to` over a copy of the server's opening setup — two statements of state change and no rule, which is why it does not contradict "this client MUST NOT decide legality". **Seat 0 is 红**, and the constants are pinned to the literal `0` in a test on purpose: every other assertion is written in terms of `redSeat`/`blackSeat`, so swapping the two relabels the world coherently and only that one test moves — measured. Seat names dispatch on the **game key, never the seat count**: 象棋 and 五子棋 both have two seats, so a seat-count criterion cannot tell them apart, and that is exactly the gap the original rule fell through. The opening setup is checked against the **authority**, not against another copy — `integration_test/xiangqi_test.dart` plays 炮二平五 and requires the server to accept it, having first sent a move the server must refuse (so the client is known not to pre-judge).

**A relocation goes to `MovePiece`, a separate hub method** — SignalR applies no C# optional-parameter defaults in either direction, so adding an argument to a live method is a breaking change rejected in the binding layer, below the log level, invisible from both ends. Whether a game relocates or places is one more thing the board registry owns, because that registry is the only place this client knows a game at all.

**`test/i18n_keys_test.dart` walks every key the code names against both bundles, and it found four holes in shipped code the day it was written** — the lobby's create button had been rendering the literal `lobby.create.submit`, and its three error paths keys that did not exist. `Translations.t` returning the key is the right behaviour and is exactly why nobody noticed. Mobile-only keys are not an option: the bundles are synced copies of web's, so the fix is always to find the key that already exists.
...
Test projects live under `backend/tests/` — read the directory rather than a list here. `Gewu.Api.Tests` **now exists** and is registered in `Gewu.slnx`; it holds pure unit tests over `Gewu.Api` types and **no host**, so the per-seat SignalR fan-out still has no end-to-end test. Its first test exists because the gap had a cost: `Clients.User(...)` needs an `IUserIdProvider`, none was registered, and every directed push (催促) went to nobody with no error at either end. The test csprojs declare `Xunit` as a global using — don't add `using Xunit;` in test files.
...
- **A strict `script-src` silently kills Angular's deferred stylesheet, and the symptom is *not* "no CSS".** The production build emits `<link rel="stylesheet" media="print" onload="this.media='all'">` (plus a `<noscript>` copy that never applies with JS on), so the global sheet only becomes active when an **inline event handler** runs. `script-src 'self'` refuses it, the sheet stays `media="print"`, and **all 137 global rules — Tailwind, theme tokens, board skins — are dead**. Angular's *component* styles are injected at runtime and still apply, so the window looks like a half-styled page rather than an unstyled one, and `document.styleSheets.length` is reassuringly non-zero. The tell is `link.media === 'print'` in the renderer. Fix by removing the *dependency* (the shell rewrites that attribute pair when serving `index.html` — deferring buys nothing off local disk), not by adding `'unsafe-inline'` to a page holding a session token. **A unit test over a hand-typed HTML string cannot catch this coming back** — `undefer-stylesheets.spec.ts` asserts against the real `dist/index.html` and goes red if Angular stops emitting the pattern.
- **A `.cmd` must be pure ASCII.** An em dash in an `echo` renders as `鈥?` in the console's default codepage — shipped once, in `start-desktop.cmd`'s own banner, and the user saw it on their first double-click.
- **`npm install` rolls the whole install back when Electron's postinstall fails**, so `node_modules/electron` can be **absent entirely** rather than just missing its 115 MB binary. A transient `ECONNRESET` is enough to trigger it, and the leftover `EPERM: rmdir` warnings are the rollback failing to finish. Telling someone to run `node node_modules/electron/install.js` in that state reports a missing module — **the two failures need different advice**, and `start-desktop.cmd` now detects them separately and fetches the runtime itself (measured: ~12 s by direct download against the URL npm was stuck on for 20 minutes).
