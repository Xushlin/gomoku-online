...
1. **`add-tetris` — the whole backend now ships; only the UI is left.** See `openspec/changes/add-tetris/`. `ScoreRun` + migration, `POST /api/score-runs`, `POST /api/score-runs/{id}/submit`, `GET /api/score-runs/leaderboard`, all verified over real HTTP. `add-web-tetris` is next: the game loop, hard-drop preview (via the public `TetrisField`), and the ladder page. Splitting it this way is what `add-xiangqi` and `add-idiom-chain` both did.
