...
| **「古谱的走法合法性」这条判据是欠着的。** 1477 条残局只过了**结构校验**(起点有子、双方交替),而 `XiangqiRules` 从标准开局重放历史,所以「逐手过规则」那条强判据在残局上**用不了** —— 188 条标准开局的走的是强路径,两条路径的强弱差别写在播种器的文档注释里。 | `XiangqiRules` 能从给定局面开局的那天(`play-from-position`)。届时把 `xiangqi-manual` 那条要求一并收紧。 |
| **古谱只做研习,界面里没有任何「你解对了」。** 不是工期:领域里**没有重复局面 / 长将 / 长捉规则**,而**和棋的定义就在那里**(六辑 1634 局里和棋 391 局);而 `IBoardGameAi.SelectMove` 只收走子历史(六个实现),从残局出发 AI 会按标准开局重建棋盘。一条源码规则钉着这件事。 | 长将 / 重复局面规则落地 **且** AI 能从给定局面走棋。**两个都到了才拆** —— 只补规则的话 AI 仍然乱下,只改 AI 的话和棋判不了。 |

Open questions waiting on the user: 红桃四 and 三带 rules (links promised). Not yet done in a browser: **the lobby's seat row** — and that is now the only one. It is covered by a test using the **real** `DefaultGameCapabilitiesService` with only the HTTP boundary stubbed, which catches a missing `ensureLoaded()` that seven stub-based tests do not; **an approximation, not a substitute.** 斗地主's play-assist, the action bar (44 px measured, 认输 clicked) and the leave guard were all driven in a real game by `fix-three-seat-resign` — **and that trip found two defects**, which is what the row above was for. `lazy-header-menu` then found a third that way, and it was the **only** way it could have been found (see the timing trap below). The harness that made it cheap: put the refresh token in `localStorage['gewu:refresh']` to land authenticated, and raise `Game__TurnTimeoutSeconds` so the game does not end while you take a few round trips. **And the pane's viewport is not always dead:** `add-xiangqi-manual` measured `innerWidth: 0` and had to defer its 375 px reading, while `add-xiangqi-endgames` got 297 and then a real 375 after `resize_window` — so **read `innerWidth` before concluding you cannot measure**, and reload at the target width because `transform` does not follow a live resize.
