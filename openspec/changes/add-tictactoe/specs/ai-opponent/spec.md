...
由此得到一条比任何启发式 AI 都强的可测性质,本实现 MUST 通过:**从任意合法局面出发、执任意一方,`TicTacToeHardAi` 永远不会输**(赢或和)。

已知后果:玩家打不赢 Hard 档。这是一字棋这个已解游戏的事实,不是缺陷 —— Easy 与 Medium 仍可战胜,难度选择器已经存在。

#### Scenario: 穷举对局永不落败(性质测试)
- **WHEN** 让 `TicTacToeHardAi` 执 X 与执 O,各自对抗一个穷举所有合法应手的对手,跑遍整棵树
- **THEN** 结果集合 MUST 只含"胜"与"和",MUST NOT 出现一次"负"
