...
两条 requirement 连标题一起改。应用顺序 RENAMED → REMOVED → MODIFIED → ADDED，
所以下面 MODIFIED 用的是新标题。

- FROM: ### Requirement: `User.RecordGameResult(GameOutcome, int newRating)` 原子更新战绩与 Rating
- TO: ### Requirement: `UserGameStats.RecordGameResult(GameOutcome, int newRating)` 原子更新某棋种的战绩与 Rating

- FROM: ### Requirement: `IUserRepository.GetTopByRatingAsync(int limit)` 返回按排行榜顺序的用户列表
- TO: ### Requirement: `IUserRepository.GetLeaderboardPagedAsync` 按棋种返回排行榜顺序
...
`add-leaderboard-pagination` 就换成了分页版 —— 规范一直落后于代码。本变更顺手改正。
