...
提案说「围观机制零改动,这次是第一次真的去验它」。验出来它坏了。

`in-room-chat` 的规则是明确的:「`Spectator` 频道**仅围观者**可见(玩家看不到围观者吐槽)」。
**写**的那一侧对(玩家发围观频道被拒,码 `spectator-channel-forbidden`);
**读**的那一侧**三条路全部泄漏**:
...
| `ChatMessage` 实时事件 | **泄漏** —— `ChatMessagePostedAsync` 分群是对的,但 `JoinSpectatorGroup` **不做任何校验**,玩家自己调它就进了围观子群 |
...
以**玩家**身份收到 2 个 RoomState 广播帧,其中围观频道消息 2 条
   泄漏 → xqfan1 : 红方这步兵进得好
   泄漏 → xqfan2 : 我押黑方赢

JoinRoom            -> OK
JoinSpectatorGroup  -> OK      ← 玩家主动把自己塞进围观子群,服务端不拦
c1 发围观评论       -> OK
玩家通过实时 ChatMessage 事件收到的围观消息: 1
   泄漏 → xqfan1 : 玩家不该看到这句 (channel=Spectator)
```

### 我先把第三条判成"正确的",而那是**读代码**读出来的结论

我最初只测了前两条,并写下「一条路做对了,而那正是另外两条一直没被发现的原因」。
那句话**是错的**,而它错得有代价:它把这个缺陷的严重性降了一级 ——
听起来像"快照路径没跟上",其实是**围观频道的保密性完全依赖客户端自觉**。

错的来源很具体:我读了 `ChatMessagePostedAsync`,看到它按频道选 group,就判它对了。
**分群确实是对的;入群没有校验。** 一个只做了一半的机制,读那一半的代码看不出来。

这是同一条教训的第三次(`add-hub-error-codes` 的 `HubException` 包装格式、
`remove-manifest-board` 的首帧、本次):**读代码得到的结论与量出来的结论,
只在它不一样的时候才有区别 —— 而它经常不一样。**

服务端今天唯一真正强制的,是"玩家不能**发**围观频道"。

**它不是本变更引入的** —— 自 `in-room-chat` 起就在,五子棋从第一天就能复现。象棋没有让它可达,
只是让它被发现:复现需要「真人对战 + 围观」同时存在,而在今天之前没有人凑出过这个组合。

**不在本变更里修**,理由是范围而不是优先级。修它要三处:
`JoinSpectatorGroup` 加成员校验、`ToState` 按调用者裁剪、`RoomState` 广播分群推两份
(今天没有"仅玩家"的 group),其中第一处顺带修正一处 spec 与实现的偏差 ——
`room-and-gameplay` 写的是 `JoinRoom` 判断身份后**额外**加子群,实现却是客户端自报。
它符合仓库规矩里的「纯 bug fix:代码不符合既有 spec,直接修,不需要新提案」,紧接本变更单独交付。
