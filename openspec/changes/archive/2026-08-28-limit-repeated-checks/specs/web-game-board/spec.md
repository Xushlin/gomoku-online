...
**「每个码在两个 locale 里都有文案」那条走查 SHALL 从这张表本身派生**,MUST NOT 手抄一份码清单。
手抄的那份**漏了三条接龙的码**,而漏掉的表现是:一个新码没有文案时,界面上会显示
`game.errors.x` 这个键本身,而没有任何断言会红 —— 那比它本要改进的通用文案更糟。
派生出来的清单 SHALL 断言非空:一张空表会让那个循环恒真。
...
#### Scenario: 长将超限有自己的说法
- **WHEN** `HubException` 消息是 `repeated-check`
- **THEN** toast 显示 `game.errors.repeated-check`,MUST NOT 显示 `game.errors.invalid-move` 或 generic
  —— 「这一步不合法」说不出「同一个将军已经三次了」,而玩家看着盘面也想不出来:
  被拒的原因不在他刚点的那两格上,而在十几手之前

#### Scenario: 码清单从表里派生
- **WHEN** 往映射表里加一个新码而忘了加两个 locale 的文案
- **THEN** 「都有文案」那条走查 MUST 红 —— 它遍历的是**表的键**,不是一份手抄的清单
