...
- [x] **那一天到了,而答案比预告的好。** `replay-every-seat` **后**合并,并且它自己把 `web-replay` 的 delta **重建在本变更之上**(`docs(openspec): 把 web-replay 的 delta 重建在 per-game-seat-labels 之上`)。所以两份 delta 不是冲突,是**层叠**:它那一份是本变更那一份的**严格超集** —— 我的两个 Scenario 一字不动地在里面,而「两位玩家」被推广成「每一个座位」。
- [x] **因此归档顺序不是礼节,是正确性**:`per-game-seat-labels` 先、`replay-every-seat` 后。反过来的话,本变更那份「恰好两位玩家」的文本会**静默盖掉**它的座位表工作 —— 而那正是「MODIFIED 是整条替换」这条规矩在防的事。归档顺序恰好也等于合并顺序,所以规矩本身够用,但**这一次是查过才知道够用的**。
