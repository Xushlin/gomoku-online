...
- [x] 7.7 Switched the rules' JSON encoder to `UnsafeRelaxedJsonEscaping` after the first E2E showed every Chinese character in the payload coming back as a 6-byte backslash-u escape sequence. Functionally fine but it inflates every response and makes logs unreadable; the generator already made the same choice.
