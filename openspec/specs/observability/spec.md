...
结构化日志与请求追溯能力:Serilog 作为日志后端(通过 `ILoggerFactory` 兼容现有 `ILogger<T>` 注入点,
业务代码无改动);每 HTTP 请求由 `X-Correlation-Id` 头标识并贯穿整个调用链(Controller → MediatR
handler → Repository → SignalR notifier → Worker);MediatR `LoggingBehavior<,>` 自动为每次 Command /
Query 记录 enter / exit / duration / exception;SignalR Hub 的 `OnConnectedAsync` / `OnDisconnectedAsync`
把 `ConnectionId` / `UserId` 推入日志 scope;日志通过 Console(本地可读)+ 滚动 JSON File(生产可
grep / 入 ELK / Loki)双通道输出。

本能力也制定**敏感字段纪律**:密码 / Refresh Token 原文及其 hash / Access Token 原文 / `User.PasswordHash`
MUST NOT 出现在任何日志输出里,由 code review 在 handler 演进时把关。
...
(`Host.UseSerilog` + `UseSerilogRequestLogging` + 管道注册)、`backend/src/Gewu.Api/Hubs/MatchHub.cs`
