...
`RoomPage` SHALL 依据 `state().gameKey` 在 `<app-board>` 与 `<app-xiangqi-board>` 之间选择，并把 `(pieceMove)` 接到 `hub.movePiece(roomId, from.row, from.col, to.row, to.col)`。

选择方式是容器模板里的 `@if`，MUST NOT 引入棋盘组件注册表 —— 理由见 design D4：这个分支已知只有两个，且没有第三个在路上（对战族只剩 成语接龙，它没有网格盘面），而注册表要用动态组件换掉类型安全的输入输出绑定。

失败路径 MUST 与既有落子一致：`HubException` 经 `hubErrorToKey` 映射成可翻译提示；并发错误额外 `getById → applySnapshot`。被拒绝后选中态 MUST 保留 —— 玩家多半想换个落点，而不是重新找那枚子。

未知棋种 MUST 退回 `<app-board>`，MUST NOT 白屏。
