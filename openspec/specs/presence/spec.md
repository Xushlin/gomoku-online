...
在线用户存在性追踪能力。基于 SignalR 连接生命周期(`OnConnectedAsync` / `OnDisconnectedAsync`)
维护内存引用计数:同一 `UserId` 的多条连接(多标签 / 多设备)算**一个在线**;最后一条断开
才变 offline。计数存在一个 `ConcurrentDictionary<UserId, int>`,`Track` 递增,`Untrack` 用
compare-and-swap 循环原子递减,归零时 `TryRemove`。

暴露两个只读 HTTP 端点供前端消费:`GET /api/presence/online-count`(大厅顶栏"当前在线 N 人")
