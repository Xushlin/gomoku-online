...
对局回放与用户战绩查询能力:把已经被 `room-and-gameplay` 持久化的 Room / Game / Move 数据,
以两个公开 HTTP 端点暴露出去:

- `GET /api/rooms/{id}/replay` —— 单局 Finished 对局的完整 moves + 元数据,供前端做"逐步回放"动画;
  Playing/Waiting 房间返回 409。
- `GET /api/users/{id}/games?page=&pageSize=` —— 按用户维度分页查战绩列表,按 `EndedAt DESC`
  排序;用 `PagedResult<T>` 通用容器包裹。

数据模型零改 —— `add-timeout-resign` 的 `EndReason` 和 `add-elo-system` 的战绩字段都已就位。
仅新增 3 个 DTO(`GameReplayDto` / `UserGameSummaryDto` / `PagedResult<T>`)、1 个应用异常
(`GameNotFinishedException` → HTTP 409)、`IRoomRepository.GetUserFinishedGamesPagedAsync` 仓储签名、
2 个 MediatR handler + 一个分页 validator,以及 `RoomsController` / `UsersController` 各一个 action。
