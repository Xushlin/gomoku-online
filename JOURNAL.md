...
- [x] **`extend-theme-tokens`** — 把棋盘那一层的视觉词汇扩到棋盘外的整个外壳,组件从「拼视觉值」改成「说角色名」。QQ 游戏皮肤的使能改动;**屏幕上只该变一处,而那一处是修复**。
  ...
  **`--shadow-elevated` 一直是个死 token,而这是本变更最大的发现。** 6 种(主题 × 明暗)组合声明了**六个不同的**阴影,全部画出同一个 `rgba(0,0,0,0.12) 0 4px 12px` —— material 浅色那份占位值。原因:Tailwind v4 的 `shadow-*` utility 不发 `box-shadow: var(--shadow-elevated)`,它走 `@property` 注册的 `--tw-shadow` 并在**构建期把 `@theme` 的值内联进去**,所以运行时 `[data-theme]` 覆盖永远到不了它。三套主题各自写的阴影**从主题系统上线起就没渲染过**,包括 `ink` 那条写在源码注释里的「阴影重 —— 活字是有厚度的」。角色 utility 直接写 `var(--shadow-elevated)`,于是修活了它;加了一条**禁止再用这个 class** 的检查,否则它会静默地死回去。**这一族值得记住的形状是:一个每套主题都认真填了值、而值从未被读取的 token,和一个填对了的 token,在任何一次代码审查里长得一模一样。**
