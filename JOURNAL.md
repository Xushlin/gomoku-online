...
- [x] **`add-xiangqi-manual`** — 《梅花谱》31 局导入,用回放的方式给玩家研习。**古谱是资料,不是对局**,所以它有自己的表、没有聚合根,而校验发生在导入那一次。

  **数据比那一页看起来的样子好得多,而这是量出来的。** 页面只是链接列表,但每条记录里带着 `DhtmlXQ` 块:`binit`(逐对比过就是标准开局摆法)、`movelist`(4 位一组的「列行列行」)、按半手挂载的注解。**所以不需要写中文记谱解析器** —— 招法本来就是坐标,与本项目只差一次转置。

  **31 局 1391 个半手全部过生产的 `XiangqiRules`,一手没被拒。** 而 `decidedEarly=0` 是那次的正面对照:坐标解码有细微错误最可能的表现就是中途冒出假将死或某一手被拒,两者都是零。变异证实了这条判据不是装饰:**去掉转置,真谱第一手当场被拒,8 条测试红并点名 `half-move 1/46`。**

  **而它推翻了一条我本来会写进校验器的假设。** 31 局里只有 **11 局**以杀棋收,**20 局**走到「优势已成」就停了 —— 所以 `result` 是**谱主的评断**,不是招法里真的走出了将死。一条「末手必须 `Decided`」的校验会把那 20 局全部拒掉,**而报出来的样子和「数据坏了」一模一样**(变异证实:7 条红);学习页也不能把它说成「将死」——那在 20 局上是错的,**而错的样子和对的样子在界面上完全一样**。两条都进了规格,各配一条会红的 Scenario。

  **架构上有一条零改动的路,而它是错的:**`ReplayPage` 其实是把 `RoomState` **合成**出来喂共享棋盘的,所以把 31 局塞成 Finished 房间零改动就能复用 `/replay/{id}`。拒绝了 —— 那会往战绩、ELO、排行榜和 `/api/users/{id}/games` 里注入 31 局没人下过的棋,而**一个写错的标记看起来和一局没人下过的棋一模一样**。两个测试钉着:播种前后 `Rooms/Games/Moves/UserGameStats` 都是 0。

  **校验放在导入而不是服务,理由是那里没有玩家的声称要判** —— 华容道之所以在服务时重放每一个走子,是因为那里有。同一条平台规则,两个方向。

  **两个「静默的空」,第一个是真踩的:**

  1. 「缺产物文件」第一版写成 `warn + return`。构建里少了一条复制规则,于是端点返回 **200 加一个空目录** —— 一次静默的空导入,而它和成功导入在接口上长得一模一样。改成抛,并把「少了复制规则会长成什么样」写进异常消息。
  2. 「不存在的谱」返回 200 空目录 —— 打错的键看起来像一部空谱。改成 404:**一部谱只因为有线路才存在**。

  **真实产物跟着测试构建走**,所以「31 条全部能过规则」每次 CI 都验一遍;它本来只在应用启动时验一次,而 CI 不启动应用。

  **scrubber 抽取:我在规格里写的「既有断言一条不改」是假的,而这是第二次了。** 有五条摸的是页面的私有 `step` / `togglePlay` / `playing`,而那些正是搬走的东西 —— **一条做不到的要求只会让人后来去改要求,而不是改代码**(`lazy-header-menu` 写过一次同样说过头的话)。搬法改成:行为的断言跟着行为走,回放页留一条**走 DOM**的接线断言(点真按钮、看棋盘那一帧变了)。拆掉 `(seek)` 绑定它会红。`ngOnDestroy` 因此可以删掉 —— **能删掉的脚手架是新做法对的证据**。

  **顺手撞出一个既有缺陷,而它是同一个坑在同一个文件里第二次出现。** 加两条无守卫路由时发现「guards every game entry route」是**恒真的**:逐条 `expect(route.canMatch).toContain(authGuard)`,而 `canMatch` 是 `undefined` 时它不失败 —— 那两条 `canMatch=undefined` 就摆在 `gameRoutes` 里,测试照样绿。而这个文件**下面那条走查的注释里已经记过一次**(那次是 `canDeactivate`)。改成数出来再比空列表,并把「故意匿名」做成 `data.publicContent` 显式标记,豁免名单用**恰好**比对。两处变异都红,而旧版在「摘掉一条真游戏路由的 authGuard」上是绿的。**修掉这个形状的一个实例,是去同一个文件里 grep 兄弟的理由。**

  **浏览器里量出来两件事,一件是缺陷、一件是假象:**

  - 第 1 手 相 从 (9,6) 走到 (7,4)、末手 23 子、回第 0 手重建成 32 子、谱评那句不含任何终局类词 —— 都对;
  - **而自动播放第一次量出来是「坏的」**:计数器停在 `1 / 46` 再也不动。面板不合成时零区变更检测不跑,`seek` 发出去、页面写了信号,**输入回不到子组件**。在等待期间手动跑变更检测之后是 0→1→2→3→4→5。**那个假象和「自动播放坏了」长得一模一样。**

  **还有一条测试桩的坑:**新加的大厅断言先是红的,而原因是 `StubGameCapabilities.sized(...)` 让页面停在加载态 —— 整列卡片都是骨架,连 AI 卡片都不在,而那才是线索。**「元素不在」的失败让你意外时,先看看有没有任何东西在。**

  **版权:** 招法与原书局名取自公有领域(王再越,清康熙;1917 石印、1926 铅印);来源页上的白话注解是近人所加的独立创作,**v1 不导入**,数据文件里记着这条判断。

  后端 **1503** 绿、前端 **955** 绿(+24),lint 0,build 0。初始包 402.62 → **409.69 kB**,两个新页面在首屏闭包里都是 **0.00 kB**(归因证明,不是因为写了 `loadComponent`)。**而 375 px 的像素测量欠着** —— 面板全程隐藏,`innerWidth` 是 0,只钉了机制;那一条在 tasks 里留着没勾,并进了延期表。

- [x] **`add-xiangqi-endgames`** — 六辑明清残局谱 **1634 局**导入,连《梅花谱》共 **1665 条、30508 半手**。**只做研习**,而不做对弈与判定的理由不是工期。

  用户给了 `/gupu/` 那一页并问「可以吗」。数据侧几乎免费:同样的 `DhtmlXQ` 格式,`99` 表示子不在盘上,所以解码器是现成的。而**「几乎免费」到此为止** —— 剩下的全是判据的问题。

  **抽样 30 局是 0 问题,而 1634 局不是 30 局的放大。全量跑完推翻了我写下的三条:**

  1. **「先走方是红」是错的** —— 7 局黑先走,全落在第一手,结果和棋 5 / 红胜 2,局面 4–11 子:正常的「黑先,红方求和求胜」题。初稿写的是「第一手不是红就拒绝并点名」,**那会拒掉 7 道合法的题,而报出来的样子和「数据坏了」一模一样**。
  2. **谱评不是「加一态」,是列的类型错了** —— **和棋 391 局**,而一个座位号表达不了和棋。「未标注」338 条也不是零散例外:**烂柯神机整辑 258 局全无结果字段**。
  3. **满盘 163 ≠ 标准开局 157** —— 有 6 局是 32 子却不是标准摆法。界面性质按子数推,校验路径按「是否恰好等于标准摆法」,两个判据不许混用。

  **而第 1 条和上一个变更的「末手必须终局」是同一个形状:一个从小样本推出的约定被写成 MUST。** 连着两次。所以现在的规矩是:**样本推出来的规则在成为 MUST 之前,先跑全集。**

  **校验被明确降级,并把「弱」写下来。** 逐手过 `XiangqiRules` 那条判据在残局上**用不了**(规则从标准开局重放,残局第一手在它看来就是非法的)。分成两条路径:标准开局 **188** 条走强路径,非标准 **1477** 条只做结构校验(起点在推演局面上有子、双方交替);推演器只搬子、不判合法性,命名与注释都不冒充规则引擎。拆除条件进了延期表 —— 因为写在规格正文里的触发条件没有人读。

  **EF 生成的迁移在 `Up` 上错了,而这是本仓库第五次为 EF 的数据搬运付账。** `RenameColumn` 把旧**值**带进新列:座位号 0(红)撞上 `Unrecorded = 0`,于是既有 31 行的「红胜」全部变成**谱未标注**、「黑胜」变成**红优**;它还给一列定长必填字段填了 `""`。**前四次都在 `Down` 上,而 `Up` 是上线才发现的那一类。** 手写 `UPDATE` 并在真库上验过(15 → 红优、16 → 黑优、31 行盘面串全部 90 字符),顺序也要紧:先 `1→2` 再 `0→1`,反了会把同一批行搬两次。

  **谱的身份加了一张表,而我在上一个变更里明确反对过它。** 当时的理由是「单行表加一次没有信息的 join」;数据从 1 部变成 7 部、属性从 0 个变成 2 个之后那个理由不成立了。**改主意的依据写在实体注释里,不只是结论。**

  **首帧的判据特意选成「子数等于起始局面的子数」,而不是「代码里传了起始局面」** —— 后者在坏版本上同样是绿的(确实传了,只是棋盘不理)。变异拆掉绑定 → 红 2 条。而同组「回到第 0 手局面逐格相同」**没有跟着红**,所以它不算这条判据的证据:它测的是每帧重建。**把两件事分清楚,比多一条绿灯值钱。**

  **浏览器里量到两件事,一件是验收、一件是缺陷:**

  - 一条 4 子残局首帧恰好 4 个子(將/士/傌/帥),傌 (1,5)→(3,4)、將 (1,3)→(2,3) 真的在动,末手剩 3 子,回第 0 手逐格相同;《梅花谱》那条仍是 32 子(旧路径没退化);341 条那辑 `h2` 数量 0(无分组层确实不画标题)。
  - **谱评那行是「The manual judges: Manual: Red better」** —— 标签与取值都带了前缀。**整套单元测试全绿**,因为它们断言 `toContain('谱评:黑优')`,而那个子串在重复后的字符串里**确实在**。只有把整句读出来才看得见。

  **而 375 px 这次量到了,上一趟欠的那笔还上了。** `add-xiangqi-manual` 读到 `innerWidth: 0` 只能欠着;这一趟先读到 297、`resize_window` 之后是真的 375,`scrollWidth == clientWidth == 375`,341 条、最长 25 字的局名换成两行、右边缘 342。**所以「量不到」是要每次重读一遍的结论,不是一劳永逸的事实。**

  版权:招法与原书局名取自明清刊本(适情雅趣 1570、橘中秘 1632、心武残编 1800…),公有领域;来源页的白话注解是近人所加的独立创作,**不导入** —— 七份产物里 0 处。抓取串行加延时、只抓一次。

  后端 **1517** 绿、前端 **967** 绿,lint 0,build 0;三个页面由归因证明不在首屏闭包里;初始包 409.69 → **409.96 kB**。

- [x] **`play-from-position`** — 从一则古谱残局开局,两个人对弈。象棋残局成为 **`xiangqi-endgame`** 这个独立棋种键,不计分。

  **内核的不变量一个字没改,而这是整件事的设计核心。** `Room` 用**类型**判断这一局要不要一份开局设置,而且**两个方向都抛**:说要却没给 → 抛,说不要却给了 → 抛。象棋「有时要、有时不要」会破坏它。三条路,前两条拒:让象棋实现 `IDealtGameRules` 而 `CreateSetup` 忽略种子(那个接口自己的文档把这种实现叫做「下一个人删不掉的东西」);把设置改成可选(那会**同时**删掉上面两个方向的检查)。取第三条:一个独立的键,于是老键**从不**要设置、新键**总是**要。

  **新接缝 `IPositionalStartRules` 与 `IDealtGameRules` 是并列的两种,不是一种的特例** —— 一个由规则从种子**生成**(发牌),一个由调用方**选定**而规则**校验**(从局面开局)。`Room` 的判断从「是不是 `IDealtGameRules`」变成「是不是这两者之一」,**而不是变成可选**。这句区别写进了 `CLAUDE.md` 那份不加载条件的内核说明里,因为它决定下一个棋种怎么设计。

  **走子逻辑没有搬家。** `XiangqiRules.Replay` 多一个「从哪块盘面起」的参数,残局那个类委托到同一份实现;`XiangqiRules` 对外行为不变。所以**既有的一千多条象棋测试一条断言没改就通过了**,那是「多一个入口没有改行为」的可执行形式 —— 而这句话上一次说过头过两次,所以这次先跑了再写。

  **两条「恰好」按设计红了,而第二条没有预告:**

  1. 不计分棋种的**恰好**集合 —— 提案里预告过。答案是**第三种理由**:一字棋「必和」,斗地主 / 挖坑「按分结算」,残局「开局就不公平」。
  2. `IFirstSeatRules` 实现**恰好一个** —— 没预告,此前只有挖坑。而那条测试的注释里**预先写好了它红的那一刻该问什么**(「这两个棋种的先手真是同一种东西吗」)。答案是「是」—— 两者都是「设置说了算」,不同的只是设置从哪来,而那由两个接口分别承载。接口不拆,名单加一个。

  **相/象的相位校验是量过全量才写的。** 1665 条起始局面里违例 0 条,士也全在九宫 X 的 5 个点上(没顺手收紧,规格只要求九宫)。**而这一条特意写清楚它是象棋的规则(象走田字且不过河),不是从样本归纳的约定** —— 前两个变更连着两次为「小样本推出的 MUST 拒掉合法数据」付账,所以现在要说清楚是哪一类。

  **建房递的是线路 id,不是盘面。** `CreateRoomCommand` 的字段被一条「**恰好**这四个」的反射断言钉住:要拦的不是 `StartPosition` 这个名字,一个叫 `Board` / `Fen` 的字段一样是那个口子。`Room.CreateFromPosition` 收的参数类型是 `IPositionalStartRules` 而不是一个棋种键,**于是「拿这条路建一局五子棋」在编译期就不成立**;反方向由 validator 拦(判据取自**类型**,不是把 `xiangqi-endgame` 写进去),而**即使它没拦住,落地的也不是一局标准开局的棋** —— 是一间开不了局的房。一间开不了局的房是难看的,但它是**响的**;一局静静从标准开局摆起的残局是不响的。

  **一条既有的反射断言按设计红了,而修法不是放宽它。** `No_dto_exposes_anything_named_setup` 禁止任何 DTO 上出现名字含 setup 的成员,理由是「斗地主的 `Game.Setup` 就是三家的底牌」。而残局房必须把起始局面下发,否则客户端把残局那几手棋叠在一副标准开局上 —— 一个看起来完全正常的错盘面。**放宽名字匹配会连它真正要拦的那件事一起放掉**,所以改成一份「恰好」的豁免名单,再把它真正在乎的那条搬到会咬人的地方:`GameSnapshotDto` 上一个 setup 都不许有。**而豁免是安全的,担保来自「它是哪个字段」,不是来自一次判断** —— 选定式那份只由 `Room.CreateFromPosition` 写,而客户端递的是一个线路 id,那条线路的起始局面 `[AllowAnonymous]` 就能读到,不给它保护不了任何东西。

  **大厅要能找到那间房,否则功能等于没做。** 残局在服务端是另一个键,而它 MUST NOT 因此变成一个独立的大厅(开一间残局房必须先选一则残局,那个大厅的「创建房间」按钮无从下手)。所以 manifest 多一个 `companionRoomKeys`,象棋大厅把两个键的房间一并列出;没声明伴生键的八个游戏仍然只发一次请求。房间行的纹章也从伴生键回到主棋种 —— 查不到的键画出**空数组**,而那不抛、不报、不红,只是不见。

  **源码规则收窄了一半,而窄的那一半有理由。** 「古谱只做研习」此前同时禁「你解对了」和「接着自己走」。前者守的是**平台不做判断**,还在;后者守的曾是「AI 从残局出发会按标准开局重建棋盘」,而人人对弈一步 AI 都不走。「和机器对弈」现在由 `IBoardGameAi.SelectMove` 的签名钉着 —— **一条源码规则守不住的东西,让类型去守。** 同时**新增一条正面规则**:残局房必须写明平台不判和棋 —— 一条「不许出现某个词」的规则守不住「必须出现某句话」。两条规则的两半都做过正控制。

  **「走法合法性」那笔欠账重新量过,而答案不是假定的那个。** 拆除条件是「`XiangqiRules` 能从给定局面开局的那天」,而这就是那天。于是 1665 条全部走强路径:**1658 条每一半手都合法、没有一条中途终局,7 条被拒**,七条理由完全相同 —— 那一手把自己的将送进被将的位置。逐一看过其中一条(`077蛛网空悬` 第 3 手 帥 (9,4)→(9,3),黑卒就在 (8,3) 上盯着),**是数据错,不是规则严**。所以欠账**只还一半**:能力有了,换路径之前要先决定那 7 条怎么办 —— 直接换会让播种在第一部谱上抛,而报出来的样子是「产物坏了」。这个结论钉成了一条「**恰好** 7 条、且每条都是自杀」的测试,而不是一句注释。

  **一次「类型检查干净」的说法什么都没测,是正控制抓到的。** `npx tsc -p tsconfig.json --noEmit` 在这个仓库编译**零个** app 文件并 exit 0(`--listFiles | grep -c src/app` = 0)。真正的类型检查走 `ng build` / `ng test` / `ng lint`。

  **浏览器里跑了一整局两人残局,而一条正控制第一次是失败的:**

  - 黑先走的线路(`firstSeat=1`):**红方 90 格全部 disabled**,黑方 5 格可点(它的 5 个子)—— 两侧都量,否则「0 可点」在任何情况下都成立;黑 將 (2,5)→(1,5) 真的落下,回合转给红。
  - 一步**在标准开局下非法、在残局下合法**的走子被接受:红俥 (1,2)→(4,2),标准开局那格是空的。
  - 反方向另开一局:黑象 (0,6)→(2,8) 的象眼上压着红兵 —— 棋盘没动、回合没转、库里只有 1 手。**目标格是空的,所以客户端确实把它发出去了**;点自己的子只会重新选子(`xiangqi-board` 判无合法性,design D2),那条路发不出任何东西,所以它当不了「服务端拒绝」的判据。
  - 认输能用:62×**44** px,CDK 确认框,`endReason=Resigned`。
  - **「这一局不进战绩」第一次是用 `GET /api/users/{id}` 读的,而那个读数什么都没证明** —— 计分的象棋局结算之后它同样是 `rating=1200, games=0`。换成 `GET /api/leaderboard?gameKey=…`:计分那局是 1220 / 1180,`xiangqi-endgame` 那张榜**零条**。**一条针对「从来不动的端点」的负向断言,是为错的理由绿的。**
  - 375 px:先读 `innerWidth`(0 → `resize_window` 后 375),`scrollWidth == clientWidth == 375`,零元素越界,那句「平台不判和棋」宽 311 px。**截图没有** —— 面板全程未显示,`screenshot` 超时,而布局读数仍然有效。

  变异:去掉 `MatchSetup` 的选定式那一支 → 2 红;handler 无视线路 id → 6 红;房间页不传起始局面 → 1 红(残局房画出 32 个子);象棋族缩回单一键 → 3 红。两次后端变异都先确认 build 0 error —— 否则「exit 1 没跑测试」会读成击杀。

  后端 **1573** 绿、前端 **993** 绿,lint 0,build 0;初始包 409.96 → **410.19 kB**(预算 480)。

- [x] **`per-game-seat-labels`** — 席位怎么称呼,由棋种自己说。象棋房不再管红方叫黑方。

  **在浏览器里量到的:** 象棋房侧栏写着「黑方:eg1」「白方:eg2」,操作条写着「白方回合」—— 而那张盘上 0 号座位画的是 **帥**。它是 `play-from-position` 收尾时顺手看见的,当时明确记为「不在范围内」并开了一个背景任务。

  **它不是象棋一家的事,也不是新缺陷。** 同一份两分支写在三处(侧栏 / 操作条 / 回放标题区),而**第四处是 grep 出来的**:成语接龙自己的对局列表也给每一手贴「黑方 / 白方」,而那个棋种连棋盘都没有。改完一处就 grep 兄弟,这次又付了钱。

  **判据错在把「有没有白方」近似成了「有几个座位」。** 现有要求的原文写着理由:「『白方走棋』在一个没有白方的棋种里是错的」—— 那句话是对的,而它被写成了座位数。象棋和成语接龙都是「没有白方的棋种」,却恰好两个座位,于是同一条理由该拦的东西从缝里过去了。**判据要贴着理由写,不要贴着当时手边那个数字写。**

  **而规格里已经写着答案,写在一个括号里 —— 没有机制。** 侧栏那条要求说「渲染「黑方 / 白方」两个座位(**象棋读作红 / 黑**)」,而同一条下面的 Scenario 说「侧栏说『黑方 / 白方』」。三处代码照着 Scenario 写。**一个写在括号里的例外,和没写是一样的。** 所以这次不是「代码没跟上规格」,是规格自己没把那句括号变成判据 —— 因此走了提案,而不是当 bug 直接改。

  **顺带发现一条 live 要求与已发布代码正好相反:** `web-doudizhu` 写着「MUST NOT 去问棋种注册表要 `seatCount`」,而 `publish-seat-count` 早就推翻了它(`seats` 只含在座的座位,等待中的三座位房间会被当成两座位),侧栏与操作条今天读的都是 `seatCount`。`validate --strict` 看不见这种矛盾。一并对齐了。

  **机制:** manifest 多一个 `seatLabelKeys`,**缺省从「黑方 / 白方」改成「座位号」** —— 旧的缺省本身就是那个失效,所以它不能继续当缺省;一个忘了声明的棋种显示「第 1 位」,不好看,但它不把红方叫成黑方。加 `GameCatalogService.byRoomKey`(匹配自己的键**或**伴生键),它同时收拢了大厅房间行里那张私拼的「伴生键 → 主棋种」表 —— 同一条解析规则此前有两份。

  **两处删掉的东西是这个判据换对了的证据:** 操作条的「座位数大于二」分支没了(三座位就是「没声明席位名」的一种),侧栏的两支合成一个循环,连带 `room.black` / `room.white` 最后两个读者一起消失;`FIRST_SEAT` 那个只为已删分支存在的常量也删了。

  **一条规则是量出来才加的。** 第一版逐格判断「这一格有没有名字」,于是一个声明两个名字、却有三个座位的棋种渲染出「黑方 / 白方 / 第 3 位」——**半边有名字半边没有,读起来像是第三个人不算玩家**。它在测试里当场露面,在浏览器里只会看起来「有点怪」。改成**全有或全无**:条数对不上就整间房说编号。

  **一条既有断言的行为被改掉了,而这里写清楚:** 描述符没到时,侧栏此前落进两座位那一支、读 `black` / `white`,画出两个人;现在座位数未知就**一个座位都不画**。从前那个答案在三座位房间里是错的(只画得出两行)而看起来完全正常。生产里看不到这一帧 —— `RoomPage.loading()` 本来就含 `!capabilities.loaded()`。

  **变异三条,全部先确认 build 0 error:** 象棋不再声明席位名 → 5 红(跨三个套件);去掉「全有或全无」→ 1 红;`byRoomKey` 退化成 `byKey` → 4 红(含大厅纹章那条 —— 它现在与席位名共用同一份解析)。

  **浏览器里三种房间都看了:** 象棋「红方:lbl1 / 黑方:lbl2」「红方回合」,整页搜不到「白方」;成语接龙「先手 / 后手」;斗地主**等待中只坐一人**时仍是「1 / 2 / 3 号座位」(那正是 `seats.length` 判据留下的坑)。375 px 用**最长**的那一种文案量(英文 "Second player" / "First player to play"):`scrollWidth == clientWidth == 375`,零元素越界。

  **不做的那一半写下来了:** `GameReplayDto` 只有 `Black` / `White`,handler 无条件读那两个字段,所以**一局已结束的斗地主,回放静默丢掉 2 号座位** —— 而没有任何测试覆盖三座位回放。那是契约的账,进了延期表,由一个单独的变更做。

  前端 993 → **1010** 绿,lint 0,build 0;初始包 410.19 → **410.49 kB**(预算 480)。后端不动。

- [x] **`replay-every-seat`** — 回放说得出每一个座位上的人。**由另一个会话做,与 `per-game-seat-labels` 并行。** 本条从它自己的 proposal / tasks 与提交里如实摘出,不是我做的,所以只写它自己记下来的东西。

  **它修的是 `per-game-seat-labels` 明确记为「不做」的那一半。** 那一半当时被记成一张欠条:回放页的实现里写着「恰好两位,而那是 DTO 的形状,不是这一处的选择」。`GameReplayDto` 只有 `Black` / `White`,handler 无条件读 `room.BlackPlayerId` / `WhitePlayerId`,于是**一局已结束的斗地主,回放静默丢掉 2 号座位**,端点仍然 200。

  **它量过才改,而且带正面对照。** 改之前用真三座位聚合跑 handler:`Black=Alice / White=Bob`,走子里的座位号是 `[0,1,2]`,而 **Carol 的 id 与用户名都不在响应里**。同一个夹具里断言 `Seats.Count == 3` —— 少了这条,「Carol 不在响应里」有可能是**夹具没坐满**而不是 handler 丢人,**两者长得一样**。

  契约改成 `Seats: IReadOnlyList<RoomSeatDto>`(复用既有形状,不新造第二种座位表示),这是一次 `!` 破坏性变更。

  **两次归档撞在同一条要求上,而它自己把顺序理顺了。** `web-replay` 的「标题区元信息」被两个未归档变更同时 MODIFIED —— 而 MODIFIED 是**整条替换**。它没有硬合,而是把自己的 delta **重建在 `per-game-seat-labels` 之上**(单独一个提交),于是那份文本成了前者的**严格超集**:席位名照旧由 manifest 给,而「两位玩家」推广成「`seats` 里的每一个座位」。**因此归档顺序不是礼节,是正确性** —— 反过来归档会让「恰好两位玩家」静默盖掉座位表那一半。顺序恰好等于合并顺序,所以既有规矩够用,**但这一次是查过才知道够用的**。

  **判据分岔写进了规格,而理由是「问题不同」:** 回放页取 `seats.length`,房间侧栏取描述符的 `seatCount` —— 回放只有 Finished 房间,坐满才开局,两者同值;侧栏面对等待中的房间,两者会分叉。**不是漏抄。**

  **浏览器里撞见一件不是它改出来的事:** 回放页标题区在 375 px 下横向溢出,而**只有 20 字符用户名(注册上限)才看得见** —— Angular 去掉元素间空白,`mx-1` 是 margin 不是断行机会,于是整行连成一个没有断点的长串(`scrollWidth 504 / clientWidth 311`)。它证明了那不是自己引入的(删掉新增的分隔符仍是 504;没碰过的房间页同样两个用户名不溢出),也解释了它为什么活到今天:**三座位那一支恰好躲过了**(标签「Seat 1」里有空格,于是有断点),而两座位是默认样本。**这是「空集合通过一切布局断言」的另一面** —— 短用户名下这条缺陷不存在,而既有测试用的都是短用户名。

  变异:后端只投影前两个座位 → 三座位两条红、两座位四条绿;前端标题区 `slice(0,2)` → 三座位两条红;删掉「这个棋种的回放还画不出来」文案 → 那条红,而失败输出里看得见缺陷本身。

  顺带对齐三处规格漂移(代码没错,规格漂了):战绩分页的过滤条件由「黑方或白方」改成「任一座位」等。

  后端 **1577** 绿,前端 **1000** 绿,lint 绿(对比度读数 1252 → **1264**,涨了),初始包与改前同。

- [x] **`fix-jwt-signing-key-env-var`** —— 生产环境那条启动异常,让运维设的是一个**运行时不读的**环境变量。纯 bug 修复,没有提案:`api-ops` 那条 live 要求早就写着 **MUST NOT 用 `GOMOKU_` 前缀**,是代码没对齐。

  **触发条件是延期表里那一行自己写的:「下一次任何规模的后端变更」。** `replay-every-seat` 是一次破坏性的 API 契约变更 —— 触发了,所以还账。

  **量过,两个方向都量:** Production 下只设 `GOMOKU_JWT__SIGNINGKEY`(异常消息让你设的正是它)→ **抛出同一条异常**;只设 `Jwt__SigningKey` → 正常启动、`Now listening on:`。所以那条消息不是措辞不准,是**照着做也不管用,而失败的样子一模一样** —— 运维手上没有任何线索说明为什么。

  **而延期表说「一处」,实际是三处:** 异常消息、`JwtOptions` 的 XML 文档、`appsettings.Development.json` 的 `_comment`。更值得记的是 `Program.cs` 里**早就有一行注释**写着「无前缀 —— `GOMOKU_` 前缀从未实现,实测被静默忽略」——`fix-spec-api-ops-env-prefix` 那次把结论写下来了,却没回头改这三处还在指错名字的地方。**一个正确的结论,和一条没改的指令,可以并排活很久。**

  **名字从 `Program.cs` 的字面量搬到了 `JwtOptions` 上的常量,理由是可断言性:** `Gewu.Api` 没有测试项目(刻意的),所以一句写在启动代码里的字符串没有任何东西守着 —— 而它就是这么错了这么久的。断言钉的是**名字与配置路径的关系**(把 `__` 换成 `:` 必须等于 `Jwt:SigningKey`),不是把同一个字符串再抄一遍:后者只在有人改名时红,证明不了改成的名字是对的。

  变异:把常量改回 `GOMOKU_JWT__SIGNINGKEY` → 两条都红,build 0 error。

  后端 1579 绿(+2)。

- [x] **`history-every-seat`** —— 战绩也说得出每一个座位,而**说不出的胜负明说说不出**。`replay-every-seat` 修了回放,战绩列表里坐着**逐字同形**的同一个缺陷:`UserGameSummaryDto` 只有 `Black` / `White`,handler 无条件读那两个字段。

  **它是被一条自带拆除条件的欠条叫醒的。** `replay-every-seat` 的 `game-replay` delta 里写着「**`UserGameSummaryDto` 的 `Black` / `White` 本变更不动,而理由要写下来**……**拆除条件:** 有人要在战绩列表里正确显示一局三人牌局的结果时」。归档时那段话原样进了 live spec,本变更就是它触发,于是那 5 行随 MODIFIED 一起删掉。**而这条欠条写在规格要求里 —— 正是 `Room.Resign` 那次被记下「一个写在规格要求里的触发条件,是没有人会读到的触发条件」的同一种位置**(那次两个三座位棋种落地都没人读到,代价是 500)。它这次生效,靠的不是机制,是**同一个人隔两个变更自己回来收**。所以它不算反例:换个人、换个季度,它和 `Room.Resign` 那条会一样躺着。真正管用的仍然是延期表 —— 这条从头到尾没进过那张表。

  **修的是三个缺陷,不是一个,而第三个只有做到一半才看得见。** ① 丢座位(与回放同形)。② **「对手」是单数** —— 三人局里另外两个人只显示得出一个,而显示哪一个取决于本人坐几号。③ **胜负是错的**:`WinnerUserId` 只装得下一个座位,而两名农民一起赢;领域层留的出路是「客户端从叫分历史里认出地主」,但**这个 DTO 刻意不含 `Moves`**,那条出路在这里不成立。于是没走出去的那个农民,自己赢的一局显示成「负」。

  **而这一条的关键是:只加 `Seats` 会把它盖起来。** 那一行会列出两个对手、看起来完全正常,然后继续说错的胜负 —— **一个看起来被修好的错误比一个明显的错误更难发现**。所以结果那一格从三支改成四支,说不出时明说 `Not recorded`。算对它需要棋种自己的阵营概念,而 `DoudizhuScoring.Settle` 至今没有生产调用方(挂在「点数阶梯」那笔账上)。**「说不出」不是拖延,它是这一行现在唯一为真的话。**

  变异三条,每条杀掉对应断言;**两座位的反面控制三次全绿** —— 少了这个,第四支有可能是一个把所有人胜负都变模糊的开关,而**那和修好了长得一样**。浏览器里以**农民**身份一页看全三支:三人局非赢家 → 两个对手 · Not recorded;两人局非赢家 → 一个对手 · Lost;三人局流局 → 两个对手 · Drew。大厅卡片与个人主页同答案。

  **顺带撞见的既有溢出,值得记的是它为什么难修:** 个人主页头部 20 字符用户名撑到 `scrollWidth 413 / clientWidth 375`,而 **`break-words` 单独无效** —— 它是 flex item,`min-width: auto` 把它钉在 max-content 宽度(实测 352 px),`overflow-wrap` 根本没有机会生效。要 `min-w-0` + `break-words` 两个类。**是回头再量一次才发现第一版没修好的**,而没修好的样子和修好了在代码里看不出区别。

  ---

  **这次真正的事故不在代码里:一个 PR 显示 `MERGED`,而它的代码从未进 `main`。** #154 的 base 是 `fix/replay-every-seat`,而 #153 已经在**一小时前**把那条分支合进了 `main`。#154 合的时候,base 分支已经不通向任何地方 —— 三个提交落进一条死分支。**GitHub 两种情况都显示绿色的 MERGED:合并确实成功了,只是没去任何地方。**

  代价是整个变更静默没有上线,而当时**唯一的异常信号(PR 上没有 CI)被解释成了「叠加 PR 的正常现象」** —— 那个解释是对的,而**一个正确的解释可以恰好盖住一个真问题**。发现它靠的是准备归档时去量 `main`,不是靠任何红色。

  **规矩:不要读 PR 的状态,去读 `main`。** `git merge-base --is-ancestor <sha> origin/main`,或者拿这次引入的符号 grep `main`。注意 **rebase-merge 会重写 SHA**,所以 `git log origin/main..origin/<branch>` 会把**已经在 `main` 里**的内容列成「不在 main」——要比**树**(`git diff --stat <main-sha> <branch-base>` 为空 = 相同),不要比 SHA。这次正是靠树相同才确认三个 cherry-pick 是无冲突重放而非重做。叠 PR 时:**底下那个最后合,或者它一落地就立刻把上面那个改指 `main`。**

  **重放之后必须重量一件事,而它差点被跳过:** 本变更的 MODIFIED delta 当初是从 `replay-every-seat` 的 **delta** 里抽的(那时它还没归档);等重新落地时 #155 已经把它归档进了 live spec。两者若有差异,MODIFIED 的**整条替换**会静默还原归档写进去的东西。逐条比对:4 条要求全部命中 live(无意外 ADD),11 行删除逐行都是有意替换的那几行。

  **归档本身也量了,而它比预测多删了 3 行。** 预测 11,实际 14:两条是 `---` 分隔符,一条是空行。**MODIFIED 的「整条替换」把要求之间那个 `---` 也算在内** —— delta 正文没以它结尾,live spec 里那一条就被删掉,而**没有任何东西会红**。恢复了 `web-lobby` 里丢掉的那一个。便宜的验法:归档前快照 spec 文件,归档后比**要求条数、`---` 条数、以及每一行删除**是否等于预测。

  **`openspec validate --strict` 的边界这次量到了底(v1.3.0)。** 做正面对照时:打断 web-lobby delta 里**全部 8 个** `#### Scenario:` → **绿**;把 web-user-profile 的 **15 个**全部打断、并把 `MODIFIED` 改成 `ADDED` → **还是绿**。而它自己的报错文案写着 "Each requirement MUST include at least one `#### Scenario:` block"。只有**整个 delta 文件没有 delta 段落标题**才会红。所以它检查的是「文件里有没有那一行 `## ADDED/MODIFIED Requirements`」,里面的内容基本不看。**而我的第一个正面对照(只打断一个 Scenario)是通过的** —— 它只是撞上了「同一条要求下还有别的 Scenario」,**一个通过的对照和一个坏掉的对照长得一模一样**,差点就此收手。

  后端 1579 → **1580** 绿,前端 1000 → **1032** 绿 / 86 文件,lint 绿(对比度读数 **1264**,与 `main` 持平 —— 覆盖没丢),初始包与改前同。
- [x] **`limit-repeated-checks`** —— 一条棋理限制:同一个将军最多重复三次,第四次不许再走。用户要的。

  **判据是局面,不是走法。** 「将军」是**局面**的性质而不是那一步的性质,所以「同一个将军送出了 N 次」与「同一个局面出现了 N 次」是同一句话 —— 实现因此不为每一步记「这一步是不是将军」,少的那份记录会和局面本身漂开。

  **变异测试改掉了一处设计,而不是补了一条测试。** 第一版把局面的身份写成「盘面 + 走出它的座位」,理由是「同一块盘面在红走完与黑走完之后是两个局面」。五条变异里四条被杀,**只有「去掉座位判断」活了下来** —— 没有任何断言能让它红。追下去发现它是**死代码**:计数只在对手被将的局面上进行,而一个「对手被将」的盘面**只可能由本方走出来**(对手走到那儿等于把自己的将留在被吃的位置,那一步早被自将规则挡掉,进不了历史)。删掉它靠的是自将必被拒,**而那条规则自己有测试**。一个靠有测试的规则支撑的删除,好过一个没有断言覆盖的分支。**一条活下来的变异可以是在说「这段代码该删」,不只是「这条测试太弱」。**

  **而另一个分支留下了,并且如实写着它没有测试。** 「将死优先于上限」撞不上:局面相同 ⇒ 合法着法集合相同 ⇒ 若此刻将死则此前那次也将死,棋早该结束了 —— 在**任何**上限值下都成立,所以「既达到上限又是将死」**构造不出来**。第一版的 Scenario 正是这么写的,那会是一条永远不会失败的断言;改成量能量的那一半(将死仍然判胜)。分支保留,理由是让正确性不依赖那段论证,而**它变成承重的那一天有名字**:谁把计数的键换成更粗的东西(例如为长捉数「这一方将了几次」)。

  **「不能再走了」不是一条悬空的规则。** 一方若只剩这一步,他确实一步也走不了 —— 而收场的东西早就在:`TurnTimeoutWorker` 判走不了的一方负,那正是传统的长将判负。同一条推理 `GameKeyValidation` 里已经用过一次。

  **`LegalMoves` 必须一起收紧,而这不是新决定:**`ai-opponent` 那条要求的 Scenario 已经写着「每一条都能被 `Apply` 接受」。所以「这一步是不是一次被禁的重复将军」抽成 `Apply` 与 `LegalMoves` **共用的一份**;搜索内部按盘面枚举的那份收不到历史,深层节点因此不懂重复 —— 那是**棋力**问题不是合法性问题,真正走出去的只有根节点那一步。

  **顺带修掉那条老陷阱的第八次出现,而它是被「加一行然后问谁覆盖它」找到的。** `hub-error.mapper.spec.ts` 那条「每个码在两个 locale 里都有文案」的走查,遍历的是一份**手抄的码清单**,**漏了三条接龙的码** —— `idiom-not-found` 有没有文案从来没被检查过,而走查的输出两种情况一模一样。改成从表派生;正面对照是删掉一个 locale 的文案,它当场红(`en is missing game.errors.repeated-check`)。另加一条「每个码都有一条显式期望」的**恰好**断言。

  **`XiangqiAi` 的报错文案改了一句话:** 原文断言「棋早该结束了」,而长将上限之后那不再是唯一解释 —— 一句断言了错误原因的报错会把下一个人送错方向。

  **文案里的「三次」与 C# 常量分处两套测试的视野之外**,没有任何断言会因为改了常量而红。触发条件写在常量的注释上,因为那是会被改的那一行。

  **推翻了一条已经写下的理由,而结论没变。** `xiangqi` 那条「界面 MUST 说清楚平台认不出和棋」的理由原文是「领域里没有重复局面 / 长将 / 长捉规则」—— 本变更之后**那句是错的**,而结论仍然对(这条规则只**拒绝一步棋**,不**宣布**结果)。改的是理由。延期表那一行的**触发条件也太宽了**:它写着「长将 / 重复局面规则落地」,而那已经落地,和棋并没有因此变成可能 —— 收紧成「长捉 / 长拦 + 一个『重复到第几次算和』的判据」。**一个写得太宽的触发条件会在自己被满足时看起来该开工。**

  **浏览器里走到了第四次,而 22 手历史是在浏览器外面攒的:** 走子只有 hub(没有 REST 落子端点),所以一个 Node 脚本用两个 token 把 22 手一次走完,浏览器只点最后那一手。线上那一帧实测是 `HubException: repeated-check`,英文 toast「The same check may be repeated at most three times, and this would be the fourth.」,中文 375 px 下「同一个将军最多只能重复三次,这一步是第四次。换一步走。」两行、311/375,零元素越界。**而这一趟最贵的不是布局,是「toast 3 秒自己消失」在不合成的窗格里读起来和「没有 toast」一模一样** —— 定时器照跑、CD 不跑,等你强制刷新时值已经没了。三处代价记进 CLAUDE.md:读信号而不是读 DOM;包 `schedule` 时要知道键是 `flash-<值>`;`delete signal.set` 会把信号**毁掉**(自有属性,没有原型兜底)。还有一条:探针的正则 `/check|将|repeat/` 匹中了房间名 `repeat-v1`,把自己的等待循环在 250 ms 时打断了 —— **探针会以答案的形状出错。**

  **而那条被推翻的理由还有第三份拷贝,是 rebase 撞出来的:** `game.endgame.no-draw` —— 残局房里给玩家看的那段字 —— 原文写着「领域里没有重复局面 / 长将 / 长捉规则」。本变更之后**那句话在界面上是假的**,而 `check-source-rules.mjs` 只查这个键存不存在、空不空,**不查它说了什么**,所以没有任何东西会红。两个 locale 一起改掉:结论(平台不判和棋)不动,理由换成「长将有上限,但判和还要靠长捉 / 长拦」。**一个理由可以有三份拷贝 —— 规格里、延期表里、和用户眼前 —— 而最后那份是唯一没有测试的。**

  后端 1580 → **1587** 绿(+7),前端 1017 → **1019** 绿,lint 绿,初始包与改前同(未动前端渲染代码)。

- [x] **`add-idiom-guess`** —— 猜成语:看释义,把成语里挖掉的字填回去。**第十个游戏**,而它是被预留好的:`IPuzzleRules` 的文档三处点名它,`PuzzlePartialResult.PayloadJson` 那个字段的原话就是「猜成语要给出处」。

  **验收判据不是这次定的,是 `puzzle-core` 早就写下等着的。** 那条要求原文:「这条此前由一个假实现验证,而假实现证明不了它…判据因此改为可执行的形式:**下一个游戏落地时,`git diff --name-only` 里 MUST NOT 出现本能力的任何文件**」。猜成语就是那个「下一个游戏」,而它做到了 —— `Puzzles/` 下既有文件改动数 **0**,新增的是一个实现 + 一处 DI 注册 + 一个 seeder 实例。**REST 面一条端点都没加**:关卡、进度、尝试、check、hint、submit 六条全是现成的。

  **而它证明不了 `IPuzzleRules` 更通用了,这句写在提案、规格和类注释三处。** 它与成语纵横**同族** —— 同一本词典、同样往格子里填字、关卡同样是提交进仓库的产物。它能证明的是**改动面**;真正的检验要等一个不填格子的关卡游戏。上一次没写这句,代价是一个假的「已证明」在规格里躺了很久,直到华容道来了把两个方法都推翻。

  **一个只有量全量才看得见的数,直接决定了设计。** 挖一个字、释义摆旁边 —— 那个字如果就写在释义里,**答案是印在题面上的**。全量 30,895 条里有释义的 12,580 条(**全部四字**,没有例外),而按「四个字里有几个不在自己释义里」分布:**0 个的有 2,914 条(23%)**,1 个 3,527,2 个 2,859,3 个 1,678,4 个 1,551。**近四分之一的条目按天真做法出出来的题,答案就在题面上** —— 抽三十条大概率一条都碰不上,而这个仓库已经为「样本推出来的 MUST」付过两次账(梅花谱、残局红先)。可用池 **9,615** 条,其中 **9,363** 条有出处 —— 剩下 **252 条没有**,所以答对载荷 MUST 容忍缺失。

  **那条规则的正面对照不是可选的。**「产物里没有一道题泄题」对一个**从不挖任何字**的实现同样是绿的,也对一条**根本没生效**的规则是绿的(只要恰好没抽到)。所以另有一条:手工造一道泄题的题,断言检查器**确实看得出来**。变异五条全杀:关掉泄题规则 → 4 红;答错也回载荷 → 1 红;`Validate` 只查第一条 → 2 红;提示忽略光标 → 1 红;计分不看错误 → 2 红。

  **两处是我写错、实现对。** 一是夹具:把「一鸣惊人」当成只有一个位置可挖,而它的释义里有「一下子」和「让人」,可挖的是两位 —— 失败信息看起来完全像实现算错了。二是提示的哨兵:我用值元组的 `default` 当「没找到」,而 `(0, 0, null)` 是**合法**的第 0 题第 0 位;隔壁 `IdiomCrosswordRules` 为同一件事留过一段注释,我照样踩了。改成下标 `-1`,并加了一条专钉第一格的测试。

  **一个 BOM 差点混进产物。** `File.WriteAllText(path, json, Encoding.UTF8)` 会写 BOM,而另外两个关卡产物都没有 —— 那三个字节让 JSON 解析器当场抛(实测 Python 直接报错),而文件在编辑器里看起来完全正常。是**拿现有产物的头三个字节比对**发现的,不是读代码发现的。

  **猜成语是最后一个 `planned` 游戏,而这件事让两条走查当场变红。** 一条是大厅的「不显示未上线的游戏」,它的守卫正是 `expect(planned.length).toBeGreaterThan(0)` —— 一条**防恒真**的断言;另一条是目录的「available 排在 planned 之前」,它拿 `indexOf` 的 `-1` 去比大小。**两条都是设计成这样的,而它们照做了**:集合空掉的那一刻红,而不是安静地什么都不验。改成 `toEqual([])` 与显式的空集分支 —— 第十一个 `planned` 游戏出现的那天它们会再红一次,那正是它们该活过来的时刻。

  顺带改掉一句**已经为假**的话:`game-emblem.ts` 写着 `t` 字形「Used by exactly two games」,而猜成语的 `?` 早就在里面 —— 三个了。手写计数,老陷阱。(旁边那条「字形不许压在填充形状上」的走查是真的、从 `GAME_REGISTRY` 派生 —— 正是当年那个 `?` 隐形留下的机制。)

  **浏览器里真打了。** 登录、12 关列表、第 1 关四题全对拿 3 星,出处纸条来自服务端载荷。为了看那**唯一一条没有出处**的题(第 8 关「牙_口清」),用 REST 把 2–8 关打通(24 星)——那也顺带证明了 `Validate` 收真提交的整条路。同一页上两半并排:「冰心一**片**」答对**有**纸条,「牙**白**口清」答对**零**纸条。答错一条 → 服务端记「答错 1 次」、不锁定、无载荷;提示停在哪一格就揭哪一格(揭了「无」并自动判对)、计「提示 1 次」。375 px:产物里最长释义 38 字、**再注入语料最长的 74 字**,`scrollWidth == clientWidth == 375`,零元素越界 —— 而先断言了屏幕上有 7 道题,**空页面零越界和布局正确长得一样**。

  后端 1588 → **1614** 绿(+26),前端 1034 → **1042** 绿(+8),lint 绿。初始包 410.19 → **411.40 kB**(+1.21,预算 480);关卡页是 **28.47 kB 懒块**。

- [x] **`add-api-base-url`** —— 客户端不再假定「服务器就在同一个源上」。**桌面壳的第一步**,而手机端将来走同一条。

  **为什么它排在 Electron 前面,是量出来的:** `frontend-web/src/app` 里绝对 URL **0 条**,REST 是 `/api/…`、hub 是 `/hubs/match`,`src/environments/` 不存在。浏览器里没问题(dev 有 proxy、生产同源),而桌面壳把页面装在本地 —— **那时没有「同源」这回事**,第一次登录就是 404,而且看起来像后端挂了。服务端那一半反而早就备好:`AddCors` + `WithOrigins` 一直在,只是从没有过一个跨域客户端,所以本变更**后端一行不改**。

  实现是一个默认值为空的 DI token + 一个拦截器。**默认空是全部要点**:拼出来仍是 `/api/rooms`,逐字节不变。

  **而这次最值钱的一条,是变异测试推翻了我自己写下的验收判据。**

  提案里写着:「既有的精确 URL 断言(`expectOne('/api/rooms')` 一类)就是『Web 端行为不变』的可执行形式」,并要求把默认值改成非空时**大面积**变红。实测:**只红了 2 条,而且两条都是本变更新写的**,既有 1042 条**全绿**。

  追下去,原因是可数的:**19 个 spec 提供了 `HttpClient`,其中只有 2 个注册了任何拦截器。** 其余的用一个光秃秃的 `provideHttpClient()` 建自己的注入器 —— 那条拦截器**根本不在它们的管线里**,它们不可能对它的任何改动有反应。**一整批「显然覆盖到了」的断言,可以整批都不在被测代码的路径上,而它们绿着,看起来正是通过的样子。** 提案的那句话是错的,规格已按实测改写。

  真正的守卫因此换了地方:跑 `APP_INTERCEPTORS`(`app.config.ts` 用的**同一份**,导出成常量而不是在测试里手抄)断言 `/api/…` 与 `/i18n/…` 都逐字不变。中间试过直接用 `appConfig.providers`,**退回来了** —— 那会启动 i18n 的 app-initializer,拆卸时留下 `EmptyError`,vitest 报「11 passed **和** 1 error」并明说「this might cause false positive tests」。`app.config.spec.ts` 的注释里早记着这道疤,我照样先踩了一次。

  **而这次的变异探针自己先错了一轮,值得记。** 第一版解析 vitest 摘要的正则被交错的 ANSI 转义序列打断,于是每次都解析成 `failed=0`,**五条变异全部「存活」—— 包括我刚看着通过的那两条**。「全存活」看起来像一个结论。修法是先剥 ANSI 再匹配,并且**先拿一条伪造的失败摘要正面对照解析器**,确认它认得出 `3 failed`,再拿它去跑。**一个坏探针会以答案的形状出错**,这个 session 里已经是第二次(上一次是一条正则匹中了房间名)。

  **顺带修掉一个只在桌面壳里才会出现的缺陷。** `authInterceptor` 用 `url.startsWith('/api/auth/login')` 判断「这条不带 token」;base 一非空,地址变成绝对的,那个判断**直接是 false** —— 登录/注册/刷新三条都会被挂上 `Authorization`,而刷新还会拿**它自己**去重试。浏览器里永远看不到。改成按**路径**匹配,两种形状都认。

  **两处刻意的边界:** 实时连接不走 `HttpClient`(拦截器碰不到),自己取一次 base —— 忘了的表现不是报错,是**棋盘再也不更新**;`/i18n/` 刻意**不算**服务器路径 —— 它是应用自己的资源,前缀掉会让应用在服务器连不上时**连字都渲染不出来**,比「登录不了」严重得多。

  变异五条全杀(默认值非空 3 红、i18n 算服务器 2 红、拦截器不前缀 3 红、hub 忘了 base 1 红、auth 退回原始匹配 1 红)。**既有 spec 文件一个都没改**(`git status` 里只有一个 `??` 新文件)。

  前端 1042 → **1053** 绿(+11),lint 绿,初始包 411.40 → **411.84 kB**。后端零改动。

- [x] **`add-desktop-shell`** —— Electron 桌面壳。**双击就开,十个游戏一个不改地带上**,第 11 个游戏仍然只写一份。

  **`app://` 而不是 `file://`,理由是 `<base href="/" />`。** `file://` 下 `/` 解析到**文件系统根**,每个 chunk 都 404,窗口一片白 —— 而那看起来像构建坏了。改哈希路由能绕开,但路由模式是 Web 端共享的,`add-api-base-url` 刚立下「Web 端一个字节不变」,不在下一个变更里给自己破例。自定义协议还顺带给一个**真正的 origin**:`localStorage`(refresh token 在里面)因此有稳定分区,CSP 也才写得出。实测在壳里 `localStorage.setItem/getItem` 正常 —— 那是 `registerSchemesAsPrivileged({ standard: true })` 挣来的。

  **本变更最有价值的一条,是它的安全测试第一版是假的。** `resolveAsset` 是壳唯一的攻击面,我写了五条穿越用例,十条全绿 —— 然后**把边界检查整个删掉,还是十条全绿**。

  追下去,是三层里前两层在干活,而我以为是第三层:`new URL().pathname` **自己就折叠了 `..`**(`app://x/../../y` 到手已经是 `/y`);`join(root, normalize(p))` 再**夹一次**(`join` 把第二个参数当相对路径,所以 `/../../../../Windows/win.ini` 落在 `<root>\Windows\win.ini`,五种形状实测全部如此);边界检查因此**够不着**。而那五条用例之所以绿,是因为假的 `exists()` 说文件不存在,于是走了 SPA 回落 —— **它们验的是回落,不是拒绝。**

  改法是两件事:让穿越那组用「**每个文件都存在**」的 `exists`,断言结果仍在 root 内;再把边界检查抽成导出的 `isInsideRoot` 单独测,**因为经由 `resolveAsset` 递不进去一个真正越界的路径**。而它值得留着的理由也量出来了:把 `join` 换成 `resolve`(一个看起来像整理的改动)会让**五种形状全部逃出去**。两段变异证明了这一点 —— `resolve` + 有检查:2 红(正常文件也服务不了,检查在拦);`resolve` + 无检查:**7 红**,多出来的 5 条正是穿越组。

  **而把窗口真的开起来,又找到一件单测和跨源浏览器验证都没找到的事。** 壳里第一个请求是 `Failed to fetch` —— 渲染进程的来源是 `app://gewu`,而服务端 CORS 白名单里只有 `http://localhost:4299`。这属于**部署**不属于代码(`AddCors` 一直都在),但不写下来没人知道,而症状是**所有数据都没有、界面却完全正常**,与「服务器挂了」一模一样。加进白名单后,壳里登录 → `app://gewu/home` → 开房间 → 资源时间线上四条请求全部指向 `localhost:5199`,**包括 `/hubs/match/negotiate`** —— 实时连接那一半也确实跟着宿主地址走。

  **第三处「测试是装饰」也在这次被变异揪出来:** 把 `API_BASE_URL` 的 factory 改回 `() => ''`,**一条都不红** —— 我的断言全都直接调 `hostApiBaseUrl()`,验的是那个函数,不是 token 在用它。而那正是壳会死掉的方式:请求退回同源变成 `app://gewu/api/…`,被壳自己的 SPA 回落当成路由,**返回 index.html 和一个 200**。没有报错、没有 404,只是数据全没了。补了一条直接断言 token 取值的用例。

  **两个探针自己先错了。** vitest 摘要的正则被 ANSI 转义打断,报出「五条变异全部存活」;`ng test` 的摘要格式又与 `vitest` 不同,`grep` 空手而归还看起来像「没有失败」。两次都是先拿一条**伪造的失败摘要**对照解析器才敢用。这个 session 里第三、第四次了 —— **坏探针会以答案的形状出错。**

  顺带:`tsconfig.spec.json` 第一版**什么都没查** —— `exclude` 会从父配置继承,子配置的 `include` 覆盖不了它。往 spec 里塞一个真的类型错误,`lint` 一声不吭。加 `"exclude": []` 才修好,而发现它的正是那次正面对照。

  **Electron 的二进制装不上,而原因是仓库里记过的那条:** 后台启动的命令没有网络出口,postinstall 的 115 MB 下载静静失败,`node_modules/electron` 停在 3.4 MB。同一个 URL 前台 `curl` **12 秒 200**。
...
- [x] **`add-mobile-shell`** —— Flutter 手机端第一刀:**外壳 + 登录 + 五子棋一个棋种**,在真 Android 模拟器上跑通(注册 → 建房 → 对手加入 → 落子 → 服务端记下那一步)。

  **代价先写在前面,因为它是用户已经做的选择而不是需要重新论证的事:** 66 个组件 / 约 18,700 行 UI、4 主题 × 明暗、3 套皮肤、音效包、第二个 SignalR 客户端要重做,而**第 11 个游戏从此是两份实现,永远**。Flutter 给的、Electron 永远给不了的只有一样:iOS 与 Google Play。

  **最大的风险先被证伪,而这是提案里写死的顺序。** `signalr_netcore` 是社区包(1.4.4,2025-09-05 发布),而 hub 走查询串 JWT + JSON 协议 + 具名方法 —— 它**可能根本不通**,而那时要换的是传输方案,一个比 UI 重写更大的决定。所以第一件事是一个**不带 UI 的 Dart 探针**:`JoinRoom` + `MakeMove` 各成功一次,并且**回头问服务端要过一次**确认那一步真的落盘了(「调用没抛」不是证据)。通了才开始写界面。

  **两样东西没有重抄,而这由机制保证:** `tool/sync_shared.dart` 把 web 端的 547 键 × 2 locale 原样拷过来,并**从 `tokens.css` 生成** 4 主题 × 明暗 × 25 个值(`var()` 全部解开,dark 与 light 合并成完整表 —— 只读 `.dark` 块会静静丢掉大半调色板)。`test/shared_sync_test.dart` 对着**真的 web 源文件**比对,判据是**键集合相等而不是包含** —— 「包含」在手机端只用了一半键时也成立。删一个键当场红。

  **而这次最值得记的,是一条绿着却几乎什么都没证明的集成测试。** 它建完房去找「我的房间」,找不到就 `orElse: () => rooms.first` —— 于是它拿着**桌面壳测试留下的那个 `desktop-check` 房间**做断言,而那个房间还是 Waiting,落子本来就会被拒。三层伪装叠在一起,输出是 `SLICE OK`。

  去掉兜底之后立刻炸出一个真 bug:**认证响应里用户名在 `user` 下,不是顶层**,所以 `auth['username']` 一直是 null,每个房间都叫 `mobile-…` —— 而那看起来只是「名字没起好」。现在测试**不许有兜底**(找不到自己的房间就是发现),并且让第二个玩家真的加入把棋局开起来,再断言**恰好一步、坐标恰好 (7,7)**。

  **三处 Android 特有的坑,全都长成「登录失败」的样子:** `10.0.2.2` 才是模拟器里宿主的回环;Flutter 模板只在 debug/profile 清单里给 `INTERNET`,**release 构建完全没有网络**;Android 9+ 默认拦截明文 HTTP,而开发服务器正是 `http://10.0.2.2:5145`。后两条是**读清单读出来的,不是构建失败报出来的** —— 明文那条只用 `network-security-config` 放开开发主机,release 仍然只走 HTTPS,不靠谁记得把开关关回去。

  **顺带一条关于镜像的反直觉结论:** 用户建议把 Gradle 源换成腾讯镜像,我照做,结果**更慢**——wrapper 按 **URL 的哈希**做缓存,换 URL 就是缓存未命中,234 MB 从零重下,445 秒后 `Premature EOF`。而原地址那份**昨天就已经下好并解包**(有 `.ok` 标记)。改回原地址即刻命中;真正有用的镜像是 Maven 那几个,它们覆盖的是依赖解析那个另外的慢阶段。**换一个下载地址会丢掉一份能用的缓存。**

  `compileSdk` 从 `flutter.compileSdkVersion`(36)钉到 **37**:`flutter_secure_storage` 的 AAR 要求它,而 Gradle 在 `:app:checkDebugAarMetadata` 上**硬失败**,不是警告。

  布局用**注册上限的 20 字符用户名**量 —— 短数据能通过一切布局断言,这仓库四次横向溢出里三次是短/空数据下看不见的。

  手机端 **17** 条单测 + 1 条端到端切片(两个都靠 `GEWU_PROBE_SERVER` 开关,没有服务器时**跳过并明说**,CI 不假绿),`flutter analyze` 零问题。Web / 桌面 / 后端**零改动**。

  不做的:另外九个棋种、iOS 构建(要 macOS,而**Flutter 的全部理由里 iOS 那一半在这台机器上验不了**)、离线、音效与棋盘皮肤、上架。

- [x] **`refactor-mobile-mvvm`** —— 手机端换成 **Dio + Provider + MVVM**,并把分层规则提炼进 `CLAUDE.md`。用户要的。

  **收益不在这一屏,在第十一屏** —— 现在这 1403 行是能跑的,所以理由写的是三个具体问题而不是「更规范」:没有模型层(到处 `raw['status']`、`move['row'] as int`,契约一改**编译器一个字不说**);业务逻辑长在 `State` 里(测它得起一个 widget);依赖靠 `widget.services` 手传。

  **而这次三个真发现,全部来自「先让它红」,没有一个是读代码读出来的。**

  **一、那条边界走查在违规会被写成的那个形状上有洞。** 我按字面量 `data/repositories/` 匹配 import,五条正面对照里四条红、**第三条活了下来** —— 因为一个真的违规者从 `data/models/` 里写的是 `../repositories/x.dart`,里面根本没有 `data/repositories/` 这几个字。规则**恰好在唯一会出现的写法上失效**,而它的输出与「没有违规」一模一样。改成先把相对导入解析成真实路径再判断,五条全红。

  **二、刷新拦截器挂在了一个永远不触发的钩子上。** `validateStatus` 放宽到 `< 500`(让 repository 查状态码而不是 catch),于是 **401 是一个「成功」的响应,`onError` 根本不会跑**。第一版把重试写在 `onError` 里,是纯死代码 —— 而症状是「请求失败了」,与它本来就会失败**完全一致**。是测试抓到的。

  **三、那条走查的第一版在自己的措辞上出错。** 它整文件搜 `BuildContext`,于是把 `login_view_model.dart` 里那句**说明它不持有 BuildContext 的注释**判成违规。一个会因为「描述规则的句子」而红的检查比没有更糟:最顺手的修法是删掉解释。现在只看非注释行。

  **顺带一件小的,同一类:** 第一版拦截器测试用了 `http_mock_adapter`,而它的路由匹配**每个桩只消费一次** —— 于是**重试**(正是被测的那个行为)找不到路由,测试因为与代码无关的原因失败。换成自己写的十五行计数 adapter,语义完全可控。

  **验收判据是既有的端到端切片,而我把自己写的那句判据改准了。** 原文是「断言一条不改」,**做不到** —— 类型搬家,取同一个事实的路径必须跟着搬(`services.username` → `deps.auth.currentUser?.username`)。改成「每一个**匹配器与期望值**逐字未变」:实测三行断言的接收者改名、**零个期望值改动**,结果仍是房间属于本人、`Playing`、恰好一步且坐标 (7,7)。**一条做不到的判据只会让人绕过它。**
...
- [x] **`add-mobile-game-catalog`** —— 手机端从「一个五子棋大厅」变成「一个有目录的平台」。象棋那一笔的地基,**象棋在这一笔里不出现**。

  **这一笔的三个真发现,两个是在实现里撞出来的,而它们都比计划里那件事更值钱。**

  **一、四个 i18n 键在已发布的代码里根本不存在,而其中一个是大厅上最显眼的按钮。** 我为了给目录屏挑键,顺手把 `lib/` 里所有 `t.t('…')` 字面量对着两个 locale 量了一遍:`lobby.create.submit`(**建房按钮**)、`lobby.errors.load-failed` / `-create-failed` / `-join-failed` 全都不在包里。手机端第一刀落地起,那个按钮渲的就是 `lobby.create.submit` 这行字面量。**`Translations.t` 找不到就返回键本身 —— 那是对的行为(洞看得见),也正是没人发现的原因**:看得见 ≠ 有人看。正确的键在包里全都有,只是名字不同(`lobby.rooms.create-button` / `lobby.errors.generic` / `lobby.create-room.errors.generic`)。加 mobile-only 键不是选项:两份包是 web 产物的同步副本,`shared_sync_test` 会红。

  于是补了 `test/i18n_keys_test.dart`,而**它自己的第一版也差点是空的**:正则要求引号紧跟 `t.t(`,于是漏掉了 `t.t(vm.registering ? 'auth.register.title' : 'auth.login.title')` —— 一个真调用、两个真键。**一个只匹配最简调用形状的正则,对其余形状什么也没说,而它的输出与「全都没问题」一模一样。** 现在按行扫,并且把那个三元调用**钉成了非空性断言**。

  **二、`GET /api/games` 返回 7 个,不是我提案里写的八个;而其中一个棋种在两个 locale 里都没有标题。** `xiangqi-endgame` 是注册过的键,但它在 web 端**也不是一个可浏览的棋种**(不在 `GAME_REGISTRY` 里,是从象棋古谱页「摆此局对弈」进的)。不过滤它,一个已发布的屏上就会有一行 `games.xiangqi-endgame.title`。

  这逼出了一条我原本没想到的判据:**唯一允许的过滤器是翻译包**,而那不是第二份表 —— 它派生自 web 同一份 i18n 产物,`shared_sync_test` 已经钉住了那份产物。三个数字分开钉(服务端 **7**、有标题 **6**、能画 **1**),**因为它们会在三种不同的变更里各自移动**:服务端注册一个新棋种、web 加一份文案、这个客户端多一个画笔。一个数字答不了三个问题。

  **三、目录仓库把 `DioException` 漏到了边界之上,而是测试问出来的。** `validateStatus` 放宽到 `< 500`,所以 4xx 是一个「成功」的响应、5xx 才让 Dio 抛。我第一版只测了 500,期望 `CatalogFailure`,实测拿到 `DioException` —— repository 没翻译它。**两侧走的是两条不同的路**,现在两侧都测。

  **说谎的那个参数,兑现了一半,而欠账写在了源码里。** `size: int` 有两处不成立:假设正方形(象棋 10×9 一个数说不出来),且星位写死 `[3, 7, 11]` —— 传别的尺寸会在盘外画点。拆成 `rows` / `cols`,几何抽成 `BoardGeometry`,装饰变成每个棋种一个 `BoardRenderer`(**共用几何,不共用装饰** —— 不是一个画笔加 `if (gameKey == …)`)。星位改成从尺寸推,并且**对着两个已知正确答案验**:15 路 `[3, 7, 11]`、19 路 `[3, 9, 15]`。**而这一笔之后 `rows != cols` 仍然只有测试在用** —— 那句话写进了类注释和 spec:指名的调用方是下一笔的象棋,**下一笔不做,这一半就该退回**。

  顺带修了一处**注释和代码对不上**:类文档说间距是 `size / (n - 1)`,代码一直是 `side / size` 加半格内缩。**代码对、注释错**,而一句写错的公式会把下一个人送错方向。

  **`cellAt` 从「夹住」改成「越界返回 null」**,这不是洁癖:非正方形盘在自己的方框里留了真实的空白边,夹住会把点在边上的一下变成一步棋 —— 一步玩家没走的棋。15×15 填满方框,所以这个情况在那里不存在,也就从没被发现。

  **四个正面对照,四个都红在指名的那一条上:** 星位改回无条件绘制 → 小盘与非正方形三条红;往棋盘注册表塞一个假条目 → 「启用条数 == 注册表」红;代码里写一个包里没有的键 → 两个 locale 各红一条;**把 `games/:key` 改成顶层路由 → 编译通过、分析零问题、`redirect` 照旧,而大厅里 `canPop()` 变 false**。最后这条是 `add-mobile-router` 那个形状的第一次复用,而它在**第二层**上同样承重 —— 现在两层都量过了。

  **既有端到端切片的判据兑现得很干净:** 改动是 1 个 import 路径、2 处 `find.byType` 的类型改名、**外加一次新点击**(首屏从大厅变成目录),**零个匹配器与期望值改动** —— `findsOneWidget`、`hasLength(1)`、`'Playing'`、`(7,7)` 一个字没动,结果仍是 `SLICE OK`。

  `const gameKey = 'gomoku'` 删了,它自己的注释写着「一个只有一项的选择器是假装成平台的选择器」—— 兑现日就是这一笔。`lib/` 里现在**除了棋盘注册表没有任何棋种字面量**,也没有任何「棋种 → 尺寸」的表(grep 过,命中全是注释)。

  单测 47 → **73**;`flutter analyze` 零问题;集成测试 5 条全绿(路由 4 + 切片 1)。Web / 桌面 / 后端零改动。

- [x] **`add-mobile-xiangqi`** —— 手机端第二个棋种。**「加一屏 = 加一个 `ui/` 目录」这句话第一次被真正检验,而它基本成立**:`lib/` 里既有的屏一个没动,新增的是一份局面重放、一个画笔、一份座位读法,和注册表里的一行。

  **判据是「跟权威比,不跟另一份副本比」,而这一笔从头到尾就靠它。** 初始摆法是服务端 `XiangqiBoard.Initial()` 的一份刻意副本 —— 单测只能拿**游戏的不变量**去量它(32 子、每方 16、各兵种数目、关于中路镜像对称、河界两行空),因为两份副本可以一起错。真正的判据在集成测试里:点这个客户端**自己认为**炮和中路所在的那两个交叉点,下出**炮二平五**,要求**服务端接受**。摆法一旦转置,那一点就落在别的子上,这一步会被拒。实测 `XIANGQI OK: (7,7) → (7,4) seat=0`。

  **而那条集成测试里非法的一步走在合法的一步前面,是故意的。** 炮 (7,7) → (5,5)(河里,不在炮能走的任何线上,且**是空格**)—— 客户端必须把它**发出去**,服务端拒,界面报错。落点故意选空格:点自己的子只会重新选中,什么都不出客户端,那就永远测不出服务端的拒绝。**这是唯一能确认这个客户端没有偷偷自己判合法性的路。**

  **重放不是判断,而这两件事很容易混。** 服务端不下发局面(`GameSnapshotDto` 只有 `Moves`),所以画盘 = 初始摆法 + 逐步 `from → to`。整个重放是两句状态变更、零条规则 —— 它连「马走日」都不知道。一个「没有原点」的 ply 被**跳过**而不是猜成从 (0,0) 出发:那正是**落子**游戏的 ply 长的样子,猜一下会把盘面悄悄弄坏,而症状读起来像重放坏了。

  **一个对照太弱,而那本身是这一笔最有意思的发现。** 我写的第一个对照是互换 `redSeat` / `blackSeat` —— 结果**只红了一条**。原因是所有断言都写成 `redSeat` / `blackSeat`,常量一起换等于世界一致地翻了个面,断言在这个变换下不变。**真正承重的不是常量,是常量与服务端座位号的绑定**:`Game` 从座位 0 开局、象棋红先,所以座位 0 **就是**红。补了一条把常量钉到字面量 `0` 的测试,再跑同一个对照 —— 三个文件各红一条。**一组只用常量表达的断言,对常量本身的错无话可说。**

  **座位读法按棋种分派,不按座位数** —— 而这是在还一笔旧账。原来那条判据写的是「座位数大于二 → 说座位号」,它的**理由**却是「『白方走棋』在一个没有白方的棋种里是错的」。象棋和五子棋**都恰好两个座位**,于是该拦的从缝里过去了。现在有一条专门的断言:两个棋种的读法**必须不同**,而它们的 `seatCount` **必须相同** —— 那正是按座位数分派做不到的事。另外半笔账:`web-game-board` 曾把「象棋读作红 / 黑」写在**括号里**,没有实现读它也没有测试守它;`seat_labels.dart` 就是当时缺的那个机制。

  **走子加方法不加参数。** `MovePiece(roomId, fromRow, fromCol, row, col)` 是另一个 hub 方法 —— SignalR 两个方向都不套用 C# 可选参数缺省值,多送少送都在**绑定层**被拒、早于任何 filter、低于日志级别,**两头都看不见**。「这个棋种是搬子还是落子」也交给棋盘注册表,因为那是这个客户端唯一知道一个棋种的地方。

  **字形是 CJK,所以量的是墨不是盒。** 行盒高约 1.45× 字号,多出来的是无字的上伸部 —— 按盒定尺寸会在上边缘产生**假失败**,只按宽度又会在对角线上「说放得下其实放不下」。要塞进圆盘的是墨的半对角线,用真的 `TextPainter` 量,14 个字形逐个过;并且配了反向对照(1.2× 字号必须超出半径),否则 `glyphFraction = 0` 也能通过。

  **越界检查是采样像素,不是读一个 bounds 对象** —— 这个 Flutter 版本的 `Picture` 没有可用的 bounds,而在推导坐标的代码旁边再推导一遍什么也没检查。栅格化后数「盘外的墨点」,375 px 与 600 px 各一次,**满盘 32 子**(空盘通过每一条布局断言)。非空性用两次渲染对比:只画线 vs 线+子,后者的墨必须是前者两倍以上 —— 否则一个「一个子都不画」的实现同样零越界。

  **五个正面对照:** RED/BLACK 互换 → 三个文件各红一条;读法改成与棋种无关 → 四条红;后排次序对调马象 → 镜像对称红;象棋改成一次点击落子 → 选中逻辑五条 + 注册表一条红;字号放大 1.2× → 墨的对照红。

  **目录里那个「恰好 1 个启用」按计划变红,而派生的集合相等断言全程绿。** 一个派生的不变量证明形状对,一个具体数字才让「数字变了」有人看见 —— 两者都要。同时 `add-mobile-game-catalog` 写在源码注释里的欠账(`rows != cols` 没有生产调用方)**在这一笔兑现**了。

  **既有端到端切片一个字节没动就过了** —— 不是「期望值没动」,是 `git diff` 为空。上一笔改了三行接收者加一次点击,这一笔零。

  单测 73 → **114**;`flutter analyze` 零问题;集成测试 6 条全绿(路由 4 + 五子棋切片 1 + 象棋 1)。Web / 桌面 / 后端零改动。

  **一个 harness 的坑记一笔:** 两个 `integration_test/` 文件放在同一次 `flutter test` 里跑,第二个会在 **loading** 阶段失败 —— 读起来像测试坏了。单独跑各自都绿。**「一次失败后面跟一次成功」与「两次成功」在输出上是一样的**,所以这条是单独确认过的,不是假设的。

  不做:排行榜(象棋计分,所以这是第一次**有可能**做;触发条件是你想看榜)、残局 / 古谱(独立的键、要 `ChosenSetup`)、人机对战(触发条件已指名是一字棋)。

- [x] **在真安卓上看一遍(纯修 bug,无提案)** —— 唯一那处「读数齐全但没人看过」。**看了不到十分钟就发现手机端的实时连接有一半从第一天起就是死的。**

  **一、订阅了一个服务端从来不发的方法名,而每一条测试都是绿的。**

  ```dart
  connection.on('RoomStateChanged', ...)   // 手机端订阅的
  ```
  ```csharp
  .SendAsync("RoomState", forSeat, ct);    // 服务端发的
  ```

  SignalR 对「订阅了一个没人调用的名字」**不报错、不警告、两头都看不见**。症状:对手加入、服务端说 `Playing`,屏幕还写「等待中」;我在手机上落了子、服务端记下了 `(7,7) → (7,4)`,**盘面一动不动**。

  **为什么一条测试都没红:每一条断言的都是「服务端」的状态(走 REST),而屏幕上那个盘是 `RoomRepository.open` 拿的那一次性 REST 快照。** 出向是通的 —— 那正是它看起来没问题的原因。桌面集成测试点了棋盘、服务端记下了走子,于是「hub 能用」被证明了一半。

  **而最初那个探针也只证明了一半。** `test/hub_probe_test.dart` 用 `JoinRoom` + `MakeMove` 建立了「这个传输层能用」—— **两个都是出向**。`mobile-shell` 那条要求写的是「SignalR 连通性 SHALL 在写任何 UI 之前被证伪」,它被一个只发不收的证明满足了。**一个只测发送的连通性证明是半个证明。**

  修法不是改一个字符串就完了:`test/hub_contract_test.dart` 从 **服务端自己的源码** 里抠出 `SendAsync("X"` 的全集,断言手机端 `connection.on('X'` 的每一个都在里面。跨语言、跨包,而 `shared_sync_test` 早有先例(它读 `frontend-web/`)。正面对照:名字改回去,两条测试红并指名那个字符串。

  顺带量到一件事:服务端发 10 个方法名,**web 端订阅的 10 个逐字对得上**;手机端订阅 2 个,其中一个不存在。web 端对得上是因为两边同时写的,不是因为有机制 —— 现在手机端有机制了。

  **二、两个只有眼睛能看见的、而且「缺键走查」看不见的问题**,因为它们都不是缺键:

  - 房间列表把 `RoomStatus.wire` 印在了屏幕上 —— 中文界面里写着英文 `Playing`。`wire` 这个名字本身就说明它是网络表示。
  - 两块屏发布了硬编码的 `Text('↻')`。包里 `lobby.errors.retry`(「重试」)一直都在。

  `test/i18n_keys_test.dart` 现在还会因为 `ui/` 下任何一个**无插值的 `Text('…')` 字面量**、以及任何一处 `.wire` 而红。两条都有正面对照。**web 端一直有「模板不许硬编码显示字符串」这条规则,手机端在真机给出两个例子之前没有机制。**

  **三、顺手删掉了一个只有我知道含义的图标。** 目录卡片上那个 ★ 表示「这个棋种计分」—— 在真机上看,它对一个玩家什么也没说,而手机端还没有排行榜可以点进去。`isRated` 仍然解析着,只是暂时没有可渲染的地方。**一个需要读代码才能理解的图标不是信息。**

  **四、返回键终于被真的按了。** `add-mobile-router` 的账里写着「安卓随后 finish activity 这一半我只是从机制推的,没在设备上按过」。按了三次:房间 → 大厅(app 在前台)、大厅 → 目录(app 在前台)、目录 → **`NexusLauncherActivity` 被 resume**(app 退出)。三次三个预期结果。**那条账清了。**

  **五、这一趟里对的东西也值得记,因为它们此前只是「量过」:** 重定向(`/` → `/login`)、静默恢复登录(重装重启后直接进目录)、三层嵌套的返回栈、象棋盘的 32 子与 14 个 CJK 字形在 1080×2400 上排得开、选中圈精确落在点中的那个子上、红/黑读法(`黑方回合` / `红方回合`)、走子标记跟着走。**推送修好之后,对手在我一下都不碰手机的情况下走了一步 —— 盘面自己变了。** 那是这一趟最想看到的一帧。

  单测 114 → **120**;`flutter analyze` 零问题。Web / 桌面 / 后端零改动。

  **留一个 spec 上的欠账,没有自己动 live spec:** 「SignalR 连通性 SHALL 在写任何 UI 之前被证伪」这条要求可以被一个**只发不收**的证明满足,而这次的代价正是那个缝。收紧它需要一笔 `fix-spec-mobile-hub-probe`,那是一次变更,不是一次 bug 修复。

- [x] **`fix-spec-mobile-hub-probe`** —— 把「连通性被证伪」收紧成**两个方向**,并给一条已经落地的走查补上要求。

  **这一笔的全部内容,是把上一笔付过的学费写成一条以后会红的判据。** `mobile-shell` 那条要求原文是「连上 `/hubs/match`、带查询串 JWT、`JoinRoom` 与 `MakeMove` 各成功一次」——**两个都是出向**;而探针自己那条自称「正面对照」的断言,是用 **REST** 查「服务端上真的有这一步棋」。**整个证明里没有一个字节是从 hub 流回客户端的**,而这条要求是满足的。

  写提案时那个探针差点又骗我一次:`grep -c "connection.on\|\.on("` 返回 **1**,读起来像「有订阅」。锚准了才看清那是 `connection.onclose` —— **连接生命周期回调,不接收服务端消息**;真正的 hub 方法订阅是 **0** 个。**结论没变,理由变准了**,而这已经是这一轮里第三次同一形状的探针出错。

  **实现只有一件事,但它有一条必须亲手跑的判据。** 探针现在在 `start()` **之前**订阅(推送可能在 join 的那一刻就到),落子之后等一条 `RoomState` 到达,断言**里面有那一步棋**、状态是 `Playing`,并且**带 10 秒超时** —— 一个永远挂着的测试和一个通过的测试从外面看一样安静。

  正面对照跑了:把订阅名改回 `RoomStateChanged`,探针在 10 秒处红,并打出那句写给下一个人的话 ——「hub 接受了我们发出去的一切,所以**出向**是通的;这是**入向**,而它在手机端第一个版本的整个生命周期里都是静默死的」。**那就是这次要防的 bug 本身。**

  顺带把 REST 那条断言的说明改准:它**留着**(它仍然有用),但不再冒充入向的证明 —— 它证明的是**服务端收到了**,不是**客户端收得到**。**两件事,而混在一起正是那个缝的形状。**

  第二条是 `ADDED`:`hub_contract_test.dart` 在修 bug 那一笔里跟着落了地,**却没有任何要求守着它**。一个有机制没要求的东西,是下一个整理代码的人第一个删掉的。要求里连实测数字一起写了:服务端 **12 个调用点、10 个不重复的名字**,web 端订阅的 10 个逐字对得上(那是同时写出来的,不是有机制),手机端订阅 **2** 个、其中 **1** 个不存在。

  **`MODIFIED` 是按行号抽出来改的,不是重打的。** 第一版用全角逗号去匹配 live 里的半角逗号 —— **一个看不见的不匹配**,`assert` 直接 0 命中。改成按行号抽取后逐行对账:live 那条要求 **10 行非空 → 5 行原封不动搬过来、5 行被替换**,与 delta 自己声明的范围一致。标题不改:「在写任何 UI 之前被证伪」对下一个传输层仍然成立,所以不需要 `RENAMED`。

  单测 **120**(探针在没有后端时仍 skip,skip 消息说明理由);带后端跑一次:`PROBE OK: started -> joined -> moved -> heard. 1 push(es) received; the first carried 1 move at (7,7)`。`CLAUDE.md` **没动** —— 那条教训在上一笔里已经写进去了,再写一遍就是两份副本。

  不动 web 端:它的 10 个名字对得上,但那是巧合不是机制。**触发条件:web 端第一次因为 hub 方法名对不上出问题。**

- [x] **`add-mobile-room-exit`** —— 手机端**从来没有真正离开过房间**,补上那条出口。

  **起点是一个数:`lib/` 里搜 `leave` / `dissolve`,七处命中全是注释里的散文,hub 的 `LeaveRoom` 是 0 次。** 按返回箭头只 pop 了一个路由 —— 服务端那边人还坐在座位上。而 `RoomRepository.close()`(会摘监听、停 hub)**没有生产调用方**。

  **于是有一个真会被看到的 bug,而它是上一笔刚刚解锁的。** 因为从不调 `LeaveRoom`,客户端留在它进过的每一个房间的 hub 分组里:进 A → 退出 → 进 B → 有人在 A 里走一步 → **你在 B 里,盘面显示 A**。这在 `fix-mobile-hub-inbound` 之前**不可能发生**,那时一条推送都到不了。**「不调 LeaveRoom」在入向是死的那段时间里是无害的,修好入向让它变成了活的** —— 一个结论可以在支撑它的前提变假之后仍然看起来没问题。

  **这一笔里我写下的两个判据都被真服务端推翻了,而单测对两个都是绿的。**

  **一、解散不是 `POST /api/rooms/{id}/dissolve`,是 `DELETE /api/rooms/{id}`。** 我写成了前者,而**旁边那条单测断言的正是这个错路径并且通过了** —— 假 adapter 接受任何 URL,断言写的是 `expect(posts, ['/api/rooms/A/dissolve'])`,**那是从客户端自己的代码推出来的**。**一个断言「客户端发出了它被写成要发的那个 URL」的测试,对「服务端有没有那个 URL」一无所知。** 只有真服务端说了 404。补了 `room_route_contract_test.dart`:从 controller 自己的 `[HttpXxx(...)]` 特性里抠出「动词 + 模板」的全集,断言客户端每一条 `/api/rooms` 调用都在里面 —— 与 `hub_contract_test` 同一手,低一层。

  **二、「离开之后那个座位空了」是错的。** 实测:**两种状态下 `/leave` 都不腾空座位**,所以普通离开**没有任何服务端可观测的变化**。判据只能是**那个分支**:房主离开等待中的房间 → 房间不见了(404);其他任何人 → 房间仍然存在。分支搞反了服务端会拒,实测三种:房主走 `/leave` 是 **409**、非房主解散是 **403**、解散进行中的房间是 **409**。

  顺带纠正一个更基础的:**两座位的房间第二个人一加入就变 `Playing`** —— 所以「等待中的房间」指的是还空着座位的那种。不写下来就会去构造一个不存在的状态,而那种测试失败时看起来像功能坏了。

  **还踩了一次这个仓库自己记着的坑,而这次它是红的。** 集成测试里我按「第一个 Waiting 的房间」取 id —— 库里有我早先用 curl 造的一堆残留房间,于是断言打在了别人的房间上。**当年那次是绿着骗过去的,这次直接红了**,改成按房主名过滤、不留 fallback。

  出口本身:`PopScope(canPop: false)` 把**系统返回键和 AppBar 的箭头收进同一个处理器** —— 这两个此前真的不同意过。进行中的对局用 `showDialog` 先问(文案 `game.leave-confirm.*` 包里全有,一个键都不用加),等待中不问,判据与 web 端同一条。**服务端拒绝时不导航** —— 离开屏幕会告诉玩家他离开了一个他还坐在里面的房间。

  `RoomDissolved` 订阅上了,收到就送回该棋种的大厅。`hub_contract_test` 的订阅数 2 → 3,走查仍然绿。**顺带记一笔:那条走查是一边倒的** —— 它只管「订阅的 ⊆ 服务端发的」,对「哪些*该*订阅」一个字都没说,而那一半没法机械推导:十个里七个不订阅是对的。

  **五个正面对照**:去掉「不是当前房间就忽略」的守卫 → 跨房间那条红;每次 open 都注册 → 「一次推送一次 republish」红;永远走解散 → 三条红;被拒当成成功 → 拒绝那条红;把解散改回错路由 → **契约走查 + 两条红**。

  单测 120 → **136**;集成测试 3 条(真后端)全绿;`flutter analyze` 零问题。Web / 桌面 / 后端零改动。

- [x] **`add-mobile-ai-room`** —— 手机端能和机器人下棋,**而一字棋是逼出这件事的那个棋种**。

  **一字棋没有新棋盘。** 服务端把它注册成 `NInARowRules("tictactoe", 3, 3, 3)`,一行胜负判定都没自己写;所以注册表里两个键指向**同一个** renderer,星位早就是从尺寸推出来的(3×3 推出空集)。这是注册表第一次不是一对一。

  **人机入口按 `supportsAi` 派生,不写清单。** 那个字段存在的理由就是这件事 —— 它的文档写着「一份手写副本的症状是**一个永远 400 的按钮**」,而这个大厅**已经有一个了**:建房 FAB 是无条件显示的,而
  `POST /api/rooms {"gameKey":"tictactoe"}` → **400 「'tictactoe' has no human-vs-human mode on this platform.」**。今天够不着(它在目录里禁用),**一有 renderer 就够得着** —— 所以两件事必须同一笔落地。

  **执白时 AI 先走,而且是异步的 —— 这是量出来的,也是这一笔最锋利的判据。** 创建响应里 `moves=0`,八秒后那个房间有 `(0,0) seat=0`。所以棋盘开局是空的,AI 那一步**只能以推送的形式到达**。于是集成测试写成:建一个执白的人机房,**此后一次输入都不给**,AI 那一子必须自己出现在**屏幕**上、回合变成我。**判据是屏幕不是服务端** —— 后者只证明服务端动了,而这个区别在 `fix-mobile-hub-inbound` 里刚付过一次学费;那一笔也因此成了这一条的**前置条件**:在它之前选执白会看到一块永远空的棋盘,读起来像「AI 坏了」。

  **两个只有我自己会发现的错,都是对照跑出来的:**

  **一、那条路由走查是按行扫的,所以漏掉了这次的调用。** `_dio.post` 把路径写在了下一行(带 body 的调用就是这么排版的),而正则要求路径紧跟在 `(` 后面 —— 于是新路由**根本没进走查的样本**,而它的输出与「全都合法」一模一样。**同一个「正则只匹配最简形状」的坑,这一轮第二次**(上一次是 i18n 走查漏掉三元调用)。改成整文件扫描,并把那条多行调用**点名**钉进非空性断言。

  **二、`identical` 那条断言测的是语言性质,不是我的意图。** 我写了「两个键共享一个 renderer」并配了对照:把 `_nInARow` 换回内联的 `GomokuRenderer()` —— **没红**,因为 Dart 会把相等的 `const` 表达式规范化成同一个实例。所以那条断言钉住的是**调用方能依赖的事实**(这两个键给你同一个 renderer),而对「map 怎么写」无话可说。真正会红的对照是把一字棋指向**另一个** renderer,而那才是值得抓的错。**把注释改准了,而不是留一条自以为在守什么的断言。**

  顺带补了一个我自己刚造出来的缺口:**`seat_labels` 里没有一字棋**,它会静默退回「座位 N」—— 那个回退对「座位没有颜色的棋种」是对的,对这个是错的。现在有一条断言:**棋盘注册表里的每个棋种都必须有读法**,所以下次启用一个棋种忘了读法会红。

  **启用一字棋顺带让 13 处断言失去了意义:** 大厅现在有两个 FAB,而 `find.byType(FloatingActionButton)` 从此有歧义(实测 `Found 2 widgets`)。全部改成**按标签找** —— 那更好:标签是玩家看见的东西,而一个指名了按钮的查找器不会悄悄开始点另一个。还有一处同类:`lobby.ai-game.button` 与 `lobby.ai-game.dialog-title` 的**文案完全相同**(都是「新建 AI 对局」),所以弹窗打开后 `find.text` 同时命中 FAB 和标题 —— 用 `find.descendant` 限定在弹窗内。**两处都不是产品缺陷,是查找器分不清。**

  目录里那个数字第二次按计划变红:**2 → 3**,而第三个**没有新增 renderer**。派生的集合相等断言两次都是绿的 —— 一个证明形状,一个让「数字动了」有人看见,两个都要。

  单测 136 → **148**;集成测试 **5 个文件**(路由 4 / 五子棋切片 1 / 象棋 1 / 离开 3 / 人机 3)全绿;`flutter analyze` 零问题。既有切片只改了查找器,**匹配器与期望值一字未动**。Web / 桌面 / 后端零改动。

  **同一条路也给五子棋和象棋开了人机对战**(实测 `gameKey=xiangqi` 是 201)—— 所以这一笔加的是「人机房」这个能力,一字棋只是第一个**必须**有它的棋种。

  不做:排行榜(一字棋 `isRated: false`,拿它当第一个榜的用例会量到一个永远空的榜)、认输、以及 AI 难度的说明文案(那是产品问题)。

- [x] **`fix-mobile-game-over`** —— 一局下完了手机端什么都不说,而客户端把同一件事**扔了两次**。

  **真机上发现的:界面停在那。** 棋盘还在,点哪儿都没反应(服务端在拒),没有一句话说赢了输了。查下去两个成因各自独立:服务端**每一份房间快照**都带 `Result` / `WinnerUserId` / `EndReason` / `EndedAt`,而 `GameSnapshot.fromJson` **只解析 `moves` 和 `currentSeat`**;hub 的 `GameEnded` 推送**订阅了**,却推进一个**没有任何人消费**的 `_errors` 流。

  **一个来源,不是两个,而这是从服务端源码量出来的。** `MakeMoveCommandHandler` 与 `ResignCommandHandler` 都是 `SaveChangesAsync` → `RoomStateChangedAsync` → `GameEndedAsync`,且 `GameEndedDto` 是从**已经写好的** `room.Game` 上取的 —— 所以结束时那份 `RoomState` 一定带着结果,`GameEnded` 对这件事是冗余的。于是**删掉**那条订阅和那条空流。**最好的机制是能被删掉的那种**,而两个来源描述同一件事正是这个仓库反复付账的形状。`hub_contract_test` 的订阅数 3 → 2,走查仍然绿(它断言的是「订阅的 ⊆ 服务端发的」,不是越多越好)。

  **文案一个键都没加。** 包里全在,而 `game.ended.dismiss`(「重新查看」)那个键**本身就定了形状**:能关掉、关掉之后还能看棋盘 —— 所以是对话框而不是常驻横幅。关掉之后不再弹:每来一条推送重弹一次比不弹更烦。

  **赢输按 id 比,不按用户名** —— 这个平台已经为「把显示名当身份」付过两次账。并且专门有一条断言:`Decided` 而 `winnerUserId` 为 null 时**不能**因为 `null == null` 读成「我赢了」。

  **顺带修了一处我自己造的不一致,而它值得写下来:** 构造函数默认 `ongoing`,`fromJson` 却把 null 解析成 `unknown`。服务端 `GameResult?` 为 null 的含义是**「还没结束」**(已知状态),而 `unknown` 该留给**不认识的取值**(不该猜的情况)。把两者 collapse 到一起会丢掉这个区别。另外 `GameResult` 是 `Ongoing=0 / Decided=1 / Draw=3` —— **没有 2**,所以按名字解析不按序号:读序号等于把一个空洞也抄过来。

  **四个正面对照**:赢输判定反转 → 四条红;不解析 `result` → 四条红;`isOver` 恒真 → 一条红;把 `GameEnded` 订阅加回去 → 那条「空流没了」红。

  单测 148 → **164**;集成测试新增一条:**一字棋 Easy 下到底**,断言结果出现在**屏幕**上、关掉之后不再弹(关掉后再点一次棋盘会产生一条推送,正是会重弹的那个情形)。判据是屏幕不是服务端 —— 这个区别在这一轮里已经付过两次学费。

  另外三条真机反馈(主题 / 棋盘颜色 / 声音、退出没确认)属于**没做**而不是坏了,单独排。

- [x] **`add-mobile-settings`** —— 手机端能选主题、选深浅、退出前会问一句,**而「换不了主题」这个报障里没有一行是新能力**。

  四个主题的 token 从第一天起就同步在 `lib/theme/tokens.g.dart` 里,`AppTheme.build` 四个都画得出来 —— 只是 `GewuApp` 往 `MaterialApp.router` 里塞的是字面量 `ink` 和 `ThemeMode.dark`。**一个够不着的能力,和一个不存在的能力,在屏幕上长得一模一样。**

  **这一笔真正的判据只有一条,而它在单测够不到的那一层。** 所有关于「存了没有」的断言 —— 缺省值、重启读回、两个轴互不干扰 —— 在**那个坏掉的版本上全是绿的**:字符串存得好好的,屏幕纹丝不动。所以对照是这么做的:在 `AppTheme.build` 顶上加一行 `name = defaultThemeName;`(**就是它出厂时的样子**),量到只有「每个主题画出可区分的 `ThemeData`」那一条红,别的一条没动。集成测试因此必须挂**外壳**而不是挂设置页:换个主题,读 `Theme.of(...)` 真的变了,而且变成了产物里那个主题的颜色 —— 不只是「变了个别的」。

  **两个轴写成两个轴,而不是八选一。** 对照:让 `setDark` 顺手写回 `defaults.themeName` —— 红的是**第二条**(「切深浅不动主题」),第一条还是绿的。这就是两个方向都要写的全部理由:**一个方向只挡一个 bug,而且不是这一个。**

  **主题名单从 `themeTokens.keys` 派生,而这条规则的对照一次红了三处。** 往产物里塞一个没有文案的 `'nameless'`:文案走查红、屏幕上出现原始 key 的那条红、「同一主题下明暗不同」也红(空 token 袋让明暗一样)。**一次变异红三处,是一个真注册表该有的形状** —— 这已经是本仓第九次修「手写清单冒充注册表」。

  **两处只有跑起来才知道的事:**

  一、**`testWidgets` 里 `await dio.post(...)` 永远不返回。** 假时钟区里 Dio 给发送超时武装的是**真** `Timer`,于是登录的 future 挂住;而 `pumpAndSettle` 推的是**假**时钟,所以 30 秒的测试超时也不会触发。症状是一个测试一路烧到 `pumpAndSettle` 那个**十分钟**的天花板 —— 读起来像「有个动画停不下来」,而不是「有个 future 卡住了」。修法是 `tester.runAsync`。顺带说明为什么不能图省事直接给 `currentUser` 赋值:`signedIn` 是只有 `_adopt` 会翻的私有 notifier,而**路由看的是 `signedIn`** —— 一个只伪造字段的夹具会让「取消没有登出」在**从来没登录过**的情况下变绿。

  二、**退出确认的标题和确认按钮是同一句文案,而这是约束的结果不是疏忽。** 这个客户端不许自造翻译键(`shared_sync_test` 就是干这个的),而共享文案里**没有**通用的「确定」、也没有退出确认的正文。于是弹窗打开后屏幕上有**三个**「退出登录」(打开它的那一行、标题、确认按钮),`find.descendant(of: AlertDialog)` 仍然命中两个 —— 查找器最后收到 `find.widgetWithText(TextButton, …)` 上。**这是本轮第三次栽在「文案重复让查找器失去指向」上**,前两次是 `lobby.ai-game.button` / `dialog-title` 和 FAB 变成两个。

  **删掉的比加上的更能说明这一笔对不对:** 退出以前是目录页一个图标、大厅一个图标,**两处都不问**;现在只有设置页一条路,`CatalogViewModel.signOut` 和 `LobbyViewModel.signOut` 双双成了死代码被删掉,`CatalogViewModel` 连 `AuthRepository` 都不再需要。**两份同一条规则的拷贝会分叉,而分叉的表现是其中一条悄悄不再问。**

  `test/view_model_notify_test.dart` 那个「恰好 N」第二次按计划红:**4 → 5**。而它顺带暴露了走查自己的一处窄:它数的是**调用** `notifyIfAlive(`,而设置页原本把它当**方法引用**传给了 `addListener`(没有括号),整个溜了过去。**走查窄是对的,该改的是代码** —— 换成一个具名的 `_onSettingsChanged`,`removeListener` 也从此没有歧义。

  「棋盘颜色」没有做成第三个轴,**而这是一条被断言钉住的判断而不是省事**:`AppTheme.boardBackground` 读的就是同一袋 token 里的 `color-well`,所以选主题**就是**选棋盘颜色 —— 有一条测试量四个主题给出的棋盘底色不止一种,哪天它们变得一样了,那才是需要独立皮肤轴的日子。声音仍然没做:这个客户端根本没有音频层,那是一笔独立的账。

  **补一条在开 PR 之后才发现的事,而它正是这一笔自己写下的那句话的反例。** 提案里说「选主题就是选棋盘颜色,因为 `boardBackground` 读同一袋 token」—— 这句话**对 token 袋是真的,对屏幕是假的**:`game_view.dart` 的调用点写的是 `boardBackground(defaultThemeName, …)`,一个**字面量**。于是四个主题下棋盘一个颜色,而那条「四个主题给出的棋盘底色不止一种」的断言**全程是绿的**,因为它问的是 token 袋不是屏幕。**一个写下来的结论不是一个量出来的结论**,哪怕它是我自己刚写的。

  修法是让棋盘颜色**骑在 `ThemeData` 上**(一个 `ThemeExtension`),而不是由调用方传一个名字进去 —— 这样它就没有第二种写错的方式:`view/` 按分层规则连仓库都够不着,问不了「现在是哪个主题」。配的机制是一条**从目录派生**的走查:`lib/ui/**/view/` 下任何文件都不许出现主题名。

  这条走查自己在写的过程中踩了两个本仓已经记过的坑,两个都被**它自己的对照**抓住:一、非空性断言报「走查到了 0 个文件」—— 路径分隔符归一化写成了会塌成空串的形式,于是 offender 列表永远是 `[]`,输出与一个干净的代码库**一模一样**;二、改对之后它命中了 `game_view.dart` 里**解释这条规则的那句注释**,而「显然的修法」是删掉解释 —— `layering_test.dart` 早就为同一件事记过一笔,这次是照着它改的(只扫代码不扫散文)。正面对照:把调用点改回字面量,那条当场红。
...
- [x] **`add-mobile-game-actions`** —— 认输与催促,**而催促这条在写第一个按钮之前就先掀出了一个服务端缺陷**。

  两件都不是新能力,是两条够不着的能力 —— 和「换不了主题」同一个形状。服务端 `POST /api/rooms/{id}/resign` 和 hub `Urge` 一直都在,手机端一个入口都没有,连 `UrgeReceived` 都没订阅过。

  **这一笔的第一件事是写一个还没有界面的探针**(`test/room_social_probe_test.dart`),一趟量四条传输(聊天 / 催促 / 围观 / 认输),而它抓到的东西写在 `fix-urge-user-routing` 里:`Clients.User(...)` 没有 `IUserIdProvider`,催促推送**从上线起寄给的是一个没人登记的地址**。**这一笔依赖那一笔** —— 否则按钮工作得完美,而什么都不会到。

  **探针能抓到,靠的是每条负面断言都配一条证明机制活着的正面断言。**「只有被催方收到」这句话里的「只有」,在**谁都没收到**时是成立的。同一趟里探针还以答案的形状错了一次:它报「围观者收不到房间频道消息」,而那是它自己漏了 `JoinRoom` —— **围观入场是三步**(`POST /spectate` → `JoinRoom` → `JoinSpectatorGroup`),中间那步正是会被跳过的那一步。两次的输出长得一样。

  **认输的显示条件有三个,而第三个在真实数据上恒真 —— 所以它必须对着一个造出来的房间断言。** `Room.Resign` 需要恰好两个座位才能指出赢家,三座位上 API 答 409;web 端曾因为客户端假设了座位数而在真实点击上返回 **500**。手机端两个棋种都是两座位,于是「`seatCount == 2`」这条判据对任何服务端会发的数据都成立 —— **一个恒真的判据是空循环**,所以测试里造了一个三座位的房间。正面对照:把 `== 2` 放宽成 `>= 2`,那条当场红(顺带另一条也红,那条钉的是「座位数读房间自己的,不是棋种目录的」—— 两者故意给了矛盾的值)。

  **认输成功之后客户端什么都不写下来**,而这条也配了对照:让 `resign()` 顺手记一个「你输了」,那条「结果只来自快照」立刻红。理由是 `fix-mobile-game-over` 已经铺好了唯一那条宣布结果的路,**两条宣布结果的路会分叉,而分叉的表现是其中一条说错了赢家**。

  **冷却不在客户端算。** 客户端只把服务端给的 429 复述回去。为了让这句话有牙,配了一条**反向**测试:一个不是冷却的失败**不许**读作冷却 —— 没有它,「所有失败都映射成冷却文案」也能通过前一条,而让人为一个连接错误等三十秒是一个**长得像正确答案的错误答案**。对照:把判断改成恒真,那条红。

  被催的计数是**计数器不是布尔**:被催第二次必须第二次可见。另外测了一条数据形状 —— `RoomStateDto` 里没有任何带 urge 的字段,所以**靠重新拉房间来发现「我被催了」的实现会永远发现不了**,推送是唯一的路。

  集成测试里对手是**脚本扮演的**(裸 REST + SignalR),因为一个 widget tree 只能有一个玩家。这也正是它有意义的原因:那条推送要穿过真 hub、寄到一个真用户 id、落到一块真屏幕上 —— 而**寄错地址正是坏掉的那一环**。认输那条则先在**等待中的房间**断言按钮不在,再在对手进来之后断言它在:否则「它出现了」证明不了「它什么时候才出现」。

  单测 184 → **201**;集成测试新增一个文件两条。`flutter analyze` 零问题。一个翻译键都没新增 —— 认输、催促、冷却、两条禁用理由的文案**早就在同步产物里**,从 web 端来的。

- [x] **`add-mobile-room-chat`** —— 房间里能说话了,**而这一笔里最容易写错的一行是「追加还是替换」**。

  服务端的聊天从第一天就在:hub `SendChat`、推送 `ChatMessage`、历史随 `RoomStateDto.chatMessages` 一起下发。手机端一样都没有。

  **服务端每次推送只带一条消息。** 一个把它当成「新的列表」的监听器,会在**任何人说第一句话的瞬间抹掉全部历史** —— 而那读起来像「历史没加载出来」,不像「我刚把它删了」。所以仓库里那一行是 `[..._chat.value, message]`,而配的对照就是把它改成 `[message]`,那条测试当场红。顺带按 id 去重,因为重连会重放。

  **三件在写代码之前量掉的事**(`test/room_social_probe_test.dart`),都属于「读注册代码不够」那一类:

  一、`ChatChannel` 是 C# 枚举,而 SignalR **拒绝一个类型不对的参数是在绑定层** —— 先于任何 filter、低于日志级别、两端都看不见。实测字符串 `"Room"` 能绑,整数 `0` 也能;这个客户端用字符串,与它解析别的枚举一致。
  二、房间快照里**确实**带 `chatMessages`,所以「进房间就看得到之前说了什么」不需要第二个接口。
  三、房间频道的推送发给**房间组**,而进那个组的方法是 `JoinRoom`,不是 `JoinSpectatorGroup`。

  **不认识的频道保持不认识,而这不是洁癖。** 把它塌成 `Room` 等于把一个平台**故意不让玩家看到**的频道广播到桌上 —— 围观频道存在的全部意义就是有些话不该到玩家眼前。对照:让 `_` 分支落到 `room`,那条当场红。

  **没有围观页签,而这是一个决定不是遗漏。** 两个页签的文案都在(从 web 同步来的),但这个客户端还不能围观,而围观频道**只有围观者收得到也只有围观者发得出** —— 在一个只有玩家到得了的屏幕上放一个围观页签,是一个**永远空的页签**,而空页签看起来像坏了。有一条从**代码**(不是散文)里查这两个键不出现的走查钉着。

  **错误映射配了反向断言。** 一个不是长度问题的失败**不许**读作长度问题 —— 没有它,「所有失败都映射成长度文案」也能通过只测长度的那条,而告诉一个人「你的消息太长了」当其实是连接断了,是一个**长得像正确答案的错误答案**。对照:把判断改成恒真,红两条。聊天的错误也和落子的错误分开放,否则一条发失败的消息会让棋盘看起来出了问题。

  界面是**底部弹出**不是并排:375 px 上棋盘已经占满宽度,侧栏要么看不清要么挤棋盘,而屏幕是为棋盘存在的。弹出层有自己的 element tree,所以里面套了一个 `ListenableBuilder` —— 否则开着面板时到的新消息要**关掉再打开**才看得见。输入框在**发送返回之后**才清空:提前清空会在被拒时把内容扔掉。

  集成测试里对手仍然是脚本扮演的,而这次的判据是**「不重开面板就看得见」**:那条消息要穿过真 hub、进房间组、被追加到一个由 REST 快照播下的列表里。

  单测 201 → **216**;集成测试新增一个文件一条。零新增翻译键。

- [x] **`add-mobile-spectate`** —— 能看别人下棋了,**而这一笔自己造了一个缺陷,是集成测试抓回来的,单测全程绿**。

  以前大厅里一局已经开打的房间点进去只有一条路:`POST /join`,而服务端会拒绝。于是「已开局的房间」在这个客户端上等于**看得见、进不去**。

  **入场是三步,中间那步是会被跳过的那一步:** `POST /spectate` → `JoinRoom` → `JoinSpectatorGroup`。房间频道的聊天和房间状态发给**房间组**,而进房间组的方法是 `JoinRoom`;`JoinSpectatorGroup` 只加围观子群。这不是推理 —— `test/room_social_probe_test.dart` 第一版就是只调了第三步,量出来是「围观者收不到桌上的话」,**读起来和一个服务端 bug 一模一样**。单测因此断言的是**顺序**不是「三个都调了」,对照是把 `open()`(它内部调 `JoinRoom`)换成 `byId()`,那条当场红。

  **只读是在 ViewModel 上拦的,不是靠界面不画。** `tap` 里多了 `isSpectator`,而配的是一条**反向**测试:同一个点,玩家点下去必须真的发出走子 —— 否则「围观者点了没发」在整条路都断掉时也成立。

  **离开走 `DELETE /spectate`,而这是服务端的规则不是手感。** 与「主持人退等待中的房间要走 `DELETE /api/rooms/{id}`」同一类。围观者也**不问**「真的要离开吗」:那句警告说的是「座位还在、计时还在走」,而围观者两样都没有 —— **一句没有后果的确认是噪音**。

  **页签的判据是「谁到得了这个频道」,不是「这个客户端支不支持围观」。** 上一笔(`add-mobile-room-chat`)因为围观没落地而只显示房间频道,理由写的是「一个只有玩家到得了的屏幕上的围观页签是一个永远空的页签」—— 那句理由**在围观落地之后对玩家仍然成立**。写成「支持围观了就都显示」的条件会在落地当天把一个空页签放到玩家面前。对照:无条件返回两个频道,玩家那两条红。

  ### 我自己造的那个缺陷,以及它为什么单测抓不到

  「还坐得下吗」我一开始写成了 `room.takenSeats < room.totalSeats`。**量过之后才知道大厅收到的根本不是那种东西**:`GET /api/rooms` 返回 `RoomSummaryDto`,**不含 `seatCount`**,而且 `seats` **只列已坐下的座位** —— 于是一个 1/2 的房间算出来是 `1 < 1 = false`,**大厅里每一个房间都变成了「观战」,包括空的**。

  单测全程绿,原因很干净:**我的夹具是一个完整房间的 JSON,带着 `seatCount`,而大厅永远收不到那种 JSON。** 一份屏幕收不到的夹具,对屏幕什么都不证明。抓到它的是 `integration_test/room_exit_test.dart` —— 一条**既有的、与围观无关的**测试:客人进不了座位,于是离开时没有确认框。

  修法是座位总数取自**棋种描述符**(大厅本来就是按棋种打开的,`gameKey` 就是它的构造参数),并且**没有描述符时给入座而不是给围观** —— 目录没加载完不是「房间满了」的证据,而给一个必然被拒的入座至少会说点什么,给「观战」是把人静默送到一个他没要求去的地方。夹具此后用**摘要的形状**,对照(把总数读回房间)让新那两条红,而**旧那份夹具让它们全绿**。

  **这一条值得记进坑表:一个在稳态下正确的判据,可能是在用一份形状不同的数据回答问题。** 「座位数」和「在座人数」在完整房间里是两个字段,在摘要里只有后者 —— 而 `seats.length` 两边都读得出来。

  单测 216 → **231**;集成测试新增一个文件一条(三个真用户:两个脚本玩家 + 一个真界面围观者,量了「围观者听得见桌上」「点棋盘服务端不多一步」「离开之后 spectators 空了」)。**十个集成测试文件逐个跑,共 19 条,全绿。** 零新增翻译键 —— 连 `lobby.rooms.watch` 都早就在同步产物里。

- [x] **`add-mobile-sound`** —— 手机端有声音了,**而这一笔的招牌是「一个听不见的人怎么验收音效」**。

  真机反馈四条里的最后一条。前三条 `add-mobile-settings` 已经答完。

  **这是这个客户端第一次遇到「从 web 拉不动」的东西。** i18n 和主题 token 都是同步产物,`shared_sync_test` 盯着不许漂;而声音**没有可同步的东西** —— web 的三个包是用 WebAudio **现场合成**的,仓库里一个音频文件都没有。所以移植的不是素材,是**设计**:哪个事件什么频率、多长、多响,那些数字明明白白写在 `packs/minimal.ts` 里。

  **而这恰好给了一个别处没有的判据:合成出来的音频就是一串数字。** 打包一个 `.wav` 只能断言它存在;生成的 PCM 可以断言长度、峰值、以及**主频** —— 用 Goertzel 直接从样本上量。所以:落子那一下**量出来是 660 Hz**,赢开在 C5(523)、输开在 G4(392)(**分别断言,不是「两者不同」** —— 后者会放过一个把它们对调的实现),催促是 880 Hz **而且中间有一段静音**(否则「两下」只是一句注释)。**没有任何一条判据依赖有人听见。**

  **三个正面对照,其中一个第一次写错了,而错法正是本仓记过的那条。** 「删掉一个事件看走查红」—— 删掉枚举值会让 `const _score` 引用不到,产出的是**编译错误**,而「exit 1、零个测试跑过」读起来和一次成功的击杀一模一样。换成一个**能编译的**合理错误实现(把一个 wire 名从 `card-play` 改成 `play-card`),那条集合相等的走查才真的红。另外两条:合成器忽略频率 → 三条音高全红;静音改成「照播」→ 那条红。

  **静音是「不去问设备」,不是「音量置零」。** 后者在一台安静的手机上和前者看起来一样,却仍然会申请音频焦点、打断别人正在听的东西。

  **事件集从 web 源码派生**(读 `sound.tokens.ts`),断言的是**集合相等**不是包含 —— 包含会在这个客户端悄悄少一个事件时变绿,而那正是手写清单的失败形状。手机端只触发其中一个子集,那不是缺口:事件集是平台级的。

  **两处在跑集成测试时差点归错因,都靠先量一下挡住了:**

  一、`play_a_move_test` 红了,而我当时正准备把它记到新插件头上 —— **实际是我的后端进程死了**(`curl` 5199 得到 `000`)。重启后同一条测试直接绿,而且顺带证明了 Windows 端带 `audioplayers` 能构建能跑。**一个失败的步骤后面跟一个成功的步骤,和两个成功的步骤长得一样** —— 这次是反过来的版本:一个环境故障长得像一个代码缺陷。

  二、`room_exit_test` 报「房间没在大厅里」。查了服务端:`GetActiveRoomsAsync` **既没有 `Take` 也没有 `OrderBy`**,所以房间**在**列表里 —— 是 `ListView` 只构建可见项,而这个 scratch 库里攒到了 **20 个**房间,新建的那个排在最后、在屏幕外。测试改成**先滚动到它再点**,因为它要证的命题从来不是「这个房间恰好在第一屏」。

  设置页现在有两个开关(深浅 + 声音),于是 `find.byType(SwitchListTile)` **从此有歧义** —— 单测和集成测试各红了一次。全部改成按标签找。**这已经是这个仓第三次栽在「同类控件多了一个,查找器失去指向」上**(两个 FAB、三个「退出登录」,现在两个开关)。

  单测 231 → **247**;集成测试 10 个文件 19 条,**逐个**跑全绿。零新增翻译键(`header.sound.*` 早就在同步产物里)。APK 已装上真机,插件启动无报错。

  **明确没做:实际听一遍。** 这一笔所有的判据都到不了「有声音出来」那一步 —— 那需要一双耳朵。

- [x] **`fix-mobile-board-jitter`** —— 「落子时界面闪动」,**而「两个棋种都有」就是它不在任何一个 renderer 里的线索**。

  真机上报的症状是五子棋和象棋落子时闪一下。原因在 `add-mobile-game-actions` 放到棋盘下方的那行「为什么现在不能催」:它携带的理由是**「当前是你的回合」** —— 轮到你为真,轮到对手为空。于是**每落一子出现/消失一次**,而棋盘在 `Expanded` 里居中,列一重排它就跟着变。

  **先把症状变成矩形,再改。** 800×600 画布上量到:轮到我 `408×408`,对手回合 `428×428` —— 每一手棋 **20 px**。同一类还有第二处、更少见:被拒的走子会在棋盘上方插一行错误文案,棋盘 408 → 364(**44 px**)。

  **这块屏幕此前一个单测都没有。** `GameView` 从来没被单独挂起来过,只有集成测试整个 app 挂过 —— 而它们从不比较一次回合变化前后的矩形。所以这个缺陷不是「测试写漏了一条」,是**这一层根本没有测试**。

  修法两条,各配一条对照(都命中预测):理由行改成 `Visibility(maintainSize: true)` —— **预留而不是删掉**,因为规格要求禁用的入口必须说明原因;错误文案改成 `Stack` 叠加。**没有改成 SnackBar**,而这是个有判据的取舍:那东西会自己消失,而 `integration_test/xiangqi_test.dart` 断言的正是那句拒绝**在屏幕上** —— `pumpAndSettle` 会把它推到消失。

  **写这条测试本身踩了两个坑,两个都记过:**

  一、四条测试全部「did not complete」—— `testWidgets` 的假时钟区里 `await catalog.load()` 永远不返回(Dio 给发送超时武装的是真 `Timer`)。挪进 `tester.runAsync`。**这是同一个坑第二次**,上一次是 `settings_test` 的登录夹具。

  二、改完之后棋盘**根本不出现**。原因是 `_board` 读的是 `Theme.of(context).extension<BoardColors>()!`(#188 挂到 `ThemeData` 上的),而测试用的是默认 `MaterialApp` 主题 → null → `!` 在 build 里抛 → 屏幕上什么都没有。**读起来像「这屏坏了」,实际是夹具不对。** 用 `AppTheme.build` 就好了 —— 顺带说明那个 `!` 是有价值的:它和「view 里不许出现主题名」那条走查合起来,逼着调用方用真主题。

  还有一处小的:第一版失败信息只报了纵向位移,于是打印出「moved by 0.0 px」**而矩形明明变了**(变的是尺寸)。**一条以答案形状出错的失败信息**,改成同时报尺寸差。

  单测 247 → **251**;`integration_test` 里 xiangqi / play_a_move / game_actions 三个直接相关的逐个跑全绿。**没做:真机上再看一眼** —— 判据是矩形,不是眼睛。

- [x] **`add-mobile-board-skins`** —— 棋盘风格能换了,**而「和 web 端一样」被做成了可以量的东西**。

  需求是一句「棋盘的风格应该能改,和 web 端一样」。我先报了一个限制(CSS 多层渐变搬不过来,只能搬调色板),被明确回绝:**要像素级**。那不是一个可以绕过去的分歧,于是它从「做不到」变成了工作量。

  **第一件事是量范围,而量完之后范围小了一个数量级。** web 每个皮肤块 43 个变量、11 个是渐变 —— 但其中 8 个是斗地主的牌面绒布和华容道的块面,**手机端没有那些棋种**。真正要画的纹理只有 3 个:棋盘底(4 层)和两种棋子(各 1 层单圆渐变)。

  **三个皮肤是三种不同的东西,这也是量出来的:** `wood` / `midnight` 是字面色 + 渐变;而 `classic` **整个是 `var(--color-*)` 和 `color-mix()`,`board-bg-image: none`** —— 它就是「跟随主题」,也就是手机端在这一笔之前的样子。

  ### 推翻了一条 live 要求,而不是绕开它

  规格里明写着「手机端 MUST NOT 另建一条独立的棋盘皮肤轴」。那句话是 `add-mobile-settings` 写的,自带拆除条件。这一笔走 `MODIFIED` 把它改写掉 —— **这个仓为「live 规格与已发布代码正好相反」付过 36 个提交的账**,而绕开一条 MUST NOT 加能力正是那件事的做法。

  ### 皮肤是同步产物,不是手抄

  扩了 `tool/sync_shared.dart`(它已经在解析 `[data-theme='…']`),生成 `skin_tokens.g.dart`,**渐变原文一并保留** —— 丢掉它漂移就检测不到,而它同时是测试的输入。

  ### 四个自己抓住自己的错

  一、**解析器第一版拒绝 `white`**(CSS 具名颜色不在子集里),而 `classic` 的棋子填充恰好是 `color-mix(…, white)` —— 那会让整个 classic 皮肤的棋子**没有填充**,和 web 端付过账的那个缺陷同一族。数了产物里的裸标识符:**只有 `white` 和 `black`**,补进子集,并配一条从产物派生的走查(出现第三个会红)。

  二、**周期性我拿整幅图相隔 12 px 的两个像素比相等** —— 那假设了底下是平的,而基础渐变沿 x 变化。断言必须**单独测那一层**;否则失败读起来是「花纹没重复」,而花纹是好的。

  三、**「简约」同时是主题 `system` 和皮肤 `classic` 的中文名** —— 同一屏两行同名,人靠分区标题分辨,`find.text` 分不了。断言改成按单选项的 `value`。

  四、**设置页现在比一屏高**,开关和退出掉到屏幕外,`ListView` 根本不构建它们,报「found 0 widgets」**读起来像控件不见了**。测试改成像用户一样滚动 —— 顺带又踩一个:`scrollUntilVisible` 会在每次拖动时求值,传一个 `.first` 收窄过的查找器会在目标还没构建时抛 `Bad state: No element`,**那看起来像测试框架坏了**。

  ### 判据

  钉住的两个 CSS 约定是最容易静默出错的:**`background-image` 的第一层在最上面**(Flutter 要反着画,画反了基色会盖住自己的木纹,读起来像「纹理没搬过来」),以及 **`90deg` 是朝右不是朝上**(转错 90° 仍然像个渐变)。

  「纹理真的画出来了」是**采样像素**验的:基色高光在 CSS 指定的 (28%,22%) 处量到 `#ecca8d`;竖纹单独渲染后**每 12 px 严格重复**且非匀质。三个皮肤的**画面**两两不同(不是三个解析出来的颜色两两不同 —— 后者会放过一个「解析三份、画一份」的实现)。还有一条**双向**断言:`classic` 必须跟主题变,`wood` 必须**不**跟主题变 —— 只测前者会放过「无视皮肤、永远用主题」,而那正是这一笔之前的行为。

  正面对照三条,全部命中预测:所有皮肤画同一个底 → 两条红;往 renderer 里塞回一个颜色字面量 → 走查红;切皮肤顺手重置主题 → 独立性那条红。

  单测 267 → **278**(css 10 + 渐变 6 + 皮肤 11,减去合并掉的);集成测试逐个跑通;`flutter analyze` 零问题;Android 构建 128 s 通过。**零新增翻译键** —— 三个皮肤名的文案早就在同步产物里。

  **明确没做:两端像素级比对。** 判据是「Flutter 画出来的像素符合 CSS 那份原文」,不是「和浏览器截图逐像素相同」—— 后者需要把浏览器拉进测试回路,是另一笔账。以及这次**没装真机**(线没插)。

- [x] **`add-mobile-language`** —— 手机端能切语言了,**而这是「够不着的能力」第三次**。

  两个语言的文案从第一天就同步在包里(547 键 × 2),`Translations.load` 两个都读得出来 —— `AppDependencies.build` 把 locale 写死成 `'zh-CN'` 并且只加载一次。**一个够不着的能力和一个不存在的能力,在屏幕上长得一样**:换不了主题、棋盘颜色不跟主题,现在是这个。

  **这一笔的关键不是「存下选择」,是让整棵树拿到新实例。** `Translations` 原本是 `Provider<Translations>.value` 的一个普通值,换实例不会让 `context.read` 的调用方重建;而加载是异步的,`GewuApp` 又必须保持 `StatelessWidget`(那条由 `shell_state_test` 的 tear-off 用编译器钉着)。所以重载放进一个仓库,外壳套两层 `ValueListenableBuilder`,**而 `Provider` 必须放在里层** —— 放外面就还是那个旧实例。

  动工前查过一件事,它决定了这个方案够不够:**没有任何 View 在 `initState` 里捕获 `Translations`**(全在方法里读)。否则会出现「切了语言,某几屏还是旧文字」,而那种半截状态最难看出是 bug 还是没翻译。

  **判据是屏幕上的字。** 这条写得这么死,是因为同一个错误犯过两次:主题那一笔存得完美而屏幕纹丝不动;棋盘颜色那一笔的断言问的是 token 袋不是屏幕。**正面对照量出来正是这个形状** —— 让切换只写存储不重载,**只有「屏幕上的字变了」那一条红,七条存储断言全绿**。

  设备语言只是**回退**不是来源:没选过时按设备语言(`en-US` 按语言匹配到 `en`,`zh-Hans-CN` 匹配到 `zh-CN`,不支持的落到默认),**选过之后设备说了不算** —— 否则「我设成了中文」会在某次系统更新后悄悄不成立。对照:让设备覆盖已存选择,那两条红。

  **设置页那条断言被我改对了两次,而两次的错都很有代表性:** 原来「一次数清所有单选项」量的其实是**恰好被构建了多少** —— 语言让页面又长一屏之后,皮肤那几行根本没被构建,集合里就少了它们;改成逐个 `scrollUntilVisible` 又踩另一个:**它只朝一个方向拖**,而屏幕顺序是主题→语言→皮肤,找完皮肤再找语言就滚过头,抛 `Bad state: No element`(**读起来像测试框架坏了**)。最后写成「从上往下走一遍收集,再滚回顶部」。

  单测 278 → **288**;`flutter analyze` 零问题;Android 构建 54.6 s 通过;零新增翻译键(`header.language.*` 早就在同步产物里)。

  **集成测试一度跑不成**(这台机器的 .NET 10 SDK 目录在会话中途被清空了),SDK 装回来之后补跑 —— **而它第一次是红的,红得很有价值**:那条断言写的是「先中文,切成英文之后标题应当不再是中文那句」,而**这一笔之后没存过选择时跟设备语言走**,测试宿主报 `en-US`,于是应用**一开始就是英文的**,`'Games'` 和 `'Games'` 比出来当然不满足「不相等」。失败信息读起来是「切换没起作用」,而切换根本无事可做。改成**读出当前语言、切到另一个、比前后两句话** —— 不假设起始语言,而这也正是这一笔本身带来的新事实。

  三个集成测试文件(settings / play_a_move / router)跑过,全绿。
