...
/// </para>
/// <para>
/// **本变更是 expand/contract 的 expand 一半:表建出来、既有战绩回填进来,但 <see cref="User"/> 上
/// 那五列还在,读者也还没切过来。** 所以此刻本实体上的方法尚无生产调用方 —— 那是有意的:
/// 迁移是这次改动里风险最高的部分,让它单独落地、单独被测,比和"改掉全部读者"混在一次审查里安全。
/// contract 那一半(切读者 + 删列 + <see cref="User"/> 不再持有战绩)紧随其后。
/// </para>
/// <para>
/// contract 之后 <see cref="User"/> **不保留任何镜像**(既不留"主棋种的分",也不留跨棋种聚合值)。
